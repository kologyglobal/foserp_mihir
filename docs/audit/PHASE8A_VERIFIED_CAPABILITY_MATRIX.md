...
| Accounting | Period Close P1 | Soft period close / reopen / readiness | `/accounting/period-close/**` | `modules/accounting/period-close/*`, `periodCloseService.ts` | Reuses `accounting-periods` close/reopen | `AccountingPeriod` | `finance.period.view\|manage\|close\|reopen` | FE `test:period-close`; docs `PERIOD_CLOSE_STATUS.md` | **VERIFIED_SHIPPED** (P1); Phase-2 UI **DEMO_ONLY** | CONDITIONAL | Advanced checklist screens still mock |
