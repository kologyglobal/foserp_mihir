...
- Migration `20260730121000_finance_year_end_close` — table present; marked applied (`migrate resolve` after timestamp collision with maintenance migration)
- `npx vitest run tests/finance/finance-year-end-close.test.ts --no-file-parallelism` — **8/8 PASS**, 0 failed, 0 skipped
  - FY lock blocked without year-end; preview P&L; post zeros sales/purchase into RE (profit −6000); idempotent replay; tenant 404; permission 403; period-close cross-tenant + 403
- Core finance pack (`finance-setup`, `finance-journal-posting`, `finance-journal-reversal`, `finance-posting-engine`, `period-close-hardening`, `finance-year-end-close`, `finance-ap-vendor-invoice-reversal`) — **7 files / 50 tests PASS**, 0 failed, 0 skipped
  - Includes AP cascade reversal (idempotencyKey hash fix for VarChar(128))
- Frontend `npm run test:period-close` — **PASS** (year-end paths included)
- Frontend `npm run test:money-out` — **68/68 PASS**
- Frontend `npm run test:money-in` — **73 passed, 3 failed** (pre-existing FE drift: workspace tab count ≠ 8; legacy demo receivables credit-notes/receipts route strings) — **not** a year-end regression
- Verdict: **Accounting READY WITH CONDITIONS** (Money In/Out + period lock + year-end P&L→RE). Deferred: accruals/prepaid/FX wizards; live TPP AIS/FX/intercompany.
...
- Backend `npm run typecheck` — **PASS** (exit 0)
- Frontend `npm run typecheck` (`tsc -b --noEmit`) — **PASS** (exit 0)
- `npx tsx scripts/test-maintenance-v1.ts` — **PASS**
  - Happy path `MT-000003` · ISSUE path `MT-000004` / `STM-000187` · on-hand 10→8 · cost entry `ISSUE_TO_MAINTENANCE`
  - Insufficient stock fail-closed · free-text part ticket-only
  - External contractor — **SKIP** (no vendor master) — not counted as pass
- `npx tsx scripts/sync-permissions.ts` — **Done: 0 permissions / 0 role links** (maintenance.* already present)
- Migration `20260730120000_maintenance_issue_to_spare_parts` applied (`warehouseId` + enum)
- Verdict: **Maintenance V1 READY**

---

# 2026-07-30 — Bank & Cash 5D4 SIMULATED AIS + cron

- Backend `npm run typecheck` — treasury/bank-connector files clean; unrelated concurrent-agent errors may exist under financial-years / inventory / maintenance
- Frontend `npm run typecheck` (`tsc -b --noEmit`) — **PASS** (exit 0)
- `npx vitest run tests/finance/finance-treasury-liquidity.test.ts tests/finance/finance-bank-connector-scaffold.test.ts tests/finance/finance-bank-connector-live.test.ts`
  - **Test Files 3 passed (3)**
  - **Tests 22 passed (22)** — **0 failed / 0 skipped**
  - Breakdown: liquidity **7/7**; scaffold **7/7**; live (5D2–5D4 + cron unit) **8/8**
- Coverage: SIMULATED AIS consent+sync → BANK_API; LIVE AIS still 422; scheduleCron tick sync; cron matcher unit; tenant isolation + permission 403 on scaffold
- Env: `BANK_CONNECTOR_AIS_PROVIDER` (default SIMULATED), `BANK_CONNECTOR_CRON_ENABLED`
- Note: liquidity day-close logged a unique-constraint prisma error then recovered (test still PASS) — possible shared-DB interference; not treated as a treasury defect
- Deferred (not tested as complete): live TPP AIS, FX revaluation, intercompany dual-LE

---

# 2026-07-30 — Live re-verify after MySQL up (Auth + Admin + Quality) — ALL GREEN

MySQL `fos_erp` up (Docker `fos-erp-mysql`, seed `vasant-trailers`). `npx tsx scripts/prisma-cli.ts migrate deploy` — **PASS / "No pending migrations to apply"** (171 migrations current, incl. `20260730110000_purchase_qi_parameter_checklist`).

- Backend `npm run typecheck` — **PASS** (exit 0)
- `auth-hardening` + `auth-self-service` — **2 files / 9 tests passed / 0 failed / 0 skipped**
- `admin-tenants-users-roles-smoke` — **1 file / 5 tests passed / 0 failed / 0 skipped**
- Combined auth + admin re-run — **3 files / 14 passed / 0 failed / 0 skipped**
- `admin-security-regression` — **1 file / 6 passed** (unchanged)
- Quality pack `quality-phase4a` + `quality-phase4b` + `quality-phase7b` + `purchase-qi-lifecycle` — **4 files / 23 tests passed / 0 failed / 0 skipped**
  - `quality-phase4a` **5/5**, `quality-phase4b` **5/5**, `quality-phase7b` **7/7**, `purchase-qi-lifecycle` **6/6** (checklist persist + edit-after-hold)

### Fixes that produced this evidence

- **Skip-gate bug (the reason Auth/Admin "passed" as skips):** `auth-hardening`, `auth-self-service`, `admin-tenants-users-roles-smoke` used `it.skipIf(!dbAvailable)` where `dbAvailable` was still `false` at collection time (set later in `beforeAll`), so every live case silently skipped even with MySQL up. Replaced with the top-level-await probe already used by `organisation-foundation` / `quality-phase4a`: `const dbAvailable = await prisma.$queryRaw\`SELECT 1\`.then(() => true).catch(() => false)`, and `beforeAll` now early-returns on `!dbAvailable`.
- **Migration `20260730110000_purchase_qi_parameter_checklist` was un-deployable:** index `purchase_quality_inspection_parameters_tenantId_qualityInspectionId_idx` is 71 chars → MySQL error 1059 (64-char identifier limit), failing the migration mid-file (P3018). Renamed to `qi_params_tenant_qi_idx` in both `migration.sql` and `schema.prisma` (`@@index([tenantId, qualityInspectionId], map: …)`).
- **Quality Phase 4A/4B QC gate:** both suites asserted a hard QC gate but never disabled `flexibleExecution`, whose server default is `true` and softens the gate — so `awaitingQuality` came back `false`. Fixtures now seed strict `manufacturingSettings` (`flexibleExecution: false`, `allowCloseWithoutQc: false`), matching `manufacturing-wo-complete-readiness.test.ts`.
- **Two stale Phase 4A assertions** (gate verified, expectations corrected — enforcement is real):
  - duplicate `stages/complete` on a `QC_PENDING` stage is **idempotent** by design (`stage-qc:<stageId>` key) → now asserts 200 + same inspection id + exactly 1 inspection row, instead of expecting 4xx.
  - blocked WO complete returns **409 `WO_COMPLETE_BLOCKED`** (not 400) with `FINAL_QC_REQUIRED` in the blocker detail → assertion updated to match.
- Pre-existing, out of scope: manufacturing/purchase fixture teardown logs FK-violation `prisma:error` noise (all `.catch()`-guarded, tests unaffected); leaves orphan test tenants.

---

# 2026-07-30 — FIN-CLOSE-1 purchase invoice retro cost

- Backend `npm run typecheck` — **PASS**
- `npm run test:fin-close-1-live` — **PASS**: GRN GR/IR ₹1,000; Vendor Invoice inventory retro ₹100 / PPV ₹0 while fully on hand; idempotent replay; Inventory↔GL matched; after two-unit return, reversal removes ₹80 from remaining stock and reclassifies consumed ₹20 to PPV.
- `tests/purchase-invoice-retro-cost-live.test.ts` — **4/4 PASS**: full on-hand + retry + reversal; partial consumption; fully consumed; lower-price credit delta + reversal.
- Combined `purchase-invoice-lifecycle` + live lifecycle + Inventory↔GL parity + retro-cost — **4 files / 15 tests PASS / 0 failed / 0 skipped**.
- Local `npx tsx scripts/prisma-cli.ts migrate deploy` — **PASS**, 171 migrations found, no pending. QI checklist table was missing despite an applied migration record; restored locally from the committed migration DDL before final verification.
- No Hostinger/remote migration or deployment was run.

---

# 2026-07-30 — Inventory Costing READY gate (SPA harness + Inventory↔GL)

- Backend `npm run typecheck` — **PASS**
- Frontend `npm run typecheck` (`tsc -b --noEmit`) — **PASS**
- `npx tsx scripts/prisma-cli.ts migrate deploy` — **PASS**; repaired the earlier partially applied `20260730110000_purchase_qi_parameter_checklist` attempt (rolled back its lone added column, marked failed attempt rolled back, then deployed); **169 migrations applied/current**
- Live Vitest command (FIFO layers, MA, Phase C, FIFO return restore, FIFO opening migration, GL parity, UAT-1 controlled, MA/FIFO golden paths, GL recon contracts) — **9 files passed / 21 tests passed / 0 failed / 0 skipped**
- `tests/inventory-costing-gl-recon-parity.test.ts` — **3/3 PASS**: operational RM stock = mapped RM GL, costing recon exposes live GL totals, overview links the Inventory↔GL hub; `forceBalanceAllowed=false`
- `npm run test:inventory-costing-spa-uat` — **PASS**: **9/9 checks** (overview, cost entries, valuation recon GET/run, FIFO layers, method preview, transfer cost preserve, service recon, service overview)
- Teardown emitted best-effort fixture FK cleanup warnings after PASS; no test failed and no assertion was skipped.
- Residual: optional human browser walk of `/inventory/costing/*`
- Verdict: **Inventory Costing READY**

---

# 2026-07-30 — Quality QI parameter checklist + quality suites

- Backend `npm run typecheck` — **PASS**
- Frontend `tsc -b --noEmit` — **PASS**
- Migration `20260730110000_purchase_qi_parameter_checklist` — **NOT DEPLOYED** (MySQL localhost:3306 unreachable: Prisma P1001)
- `npx vitest run tests/quality-phase4a.test.ts tests/quality-phase4b.test.ts tests/quality-phase7b.test.ts tests/purchase-qi-lifecycle.test.ts --no-file-parallelism --hookTimeout=120000`
  - **3 passed / 20 skipped** (DB pool timeout; live cases skipped)
  - Skips are **not** passes. Re-run after DB up + migrate deploy.
- New assertions in `purchase-qi-lifecycle.test.ts`: parameter create defaults, PATCH persistence, edit-after-hold.

---
# 2026-07-30 � Auth self-service + Admin IAM smoke

- FE `npm run test:admin-iam` � **PASS** (routes, bridges, profile/change-password wiring)
- FE `npm run test:uat-01-auth` � **24/24 PASS** (incl. self-service 8a�8c)
- BE suites added: `tests/auth-self-service.test.ts`, `tests/admin-tenants-users-roles-smoke.test.ts`
- Local MySQL unavailable (localhost:3306 closed) � live vitest cases skipped; start MySQL + seed then:
  - `cd backend && npm run test:auth-self-service`
  - `cd backend && npm run test:admin-iam-smoke`
- Code: password policy on user create; tenant admin password min length; duplicate admin tenant routes removed
...
|---------------|--------|
| `npm run typecheck` (backend) | **PASS** |
| `tests/crm-forecast.test.ts` | **2/2** � weighted = S(amount � probability/100) + bucket/at-risk |
