...
- **Stage completion** requires good quantity ≥ planned quantity × (1 − `underproductionTolerancePercent`/100). On completion, all of that stage's operations are marked `COMPLETED`, and any `NOT_STARTED` successor operation whose *every* mandatory predecessor is now `COMPLETED`/`SKIPPED` is promoted to `READY` (dependency-aware — an assembly stage depending on two parallel stages only becomes ready once **both** are done). The response's `promotedStages[]` lists only stages that *transitioned* to `READY` as a result of this call (not stages that were already ready).
- **Work order completion** requires every non-optional stage to be `COMPLETED`; stamps `actualCompletedAt`; performs **no** inventory/GL mutation; returns `warnings[]`.
