...
export function CrmLeadListPage() {
  ...
  const crmOwnerOptions = useCrmOwnerOptions()
  ...
  const canEdit = canPermission('sales', 'edit')
  ...
  const followUpLead = followUpLeadId ? leads.find((l) => l.id === followUpLeadId) ?? null : null
  ...
  const followUpCtx = followUpLead ? leadEngagementContext(followUpLead) : undefined
  ...
  function bulkAssignLeads(selected: ReturnType<typeof enrichLeadRow>[]) {
    const owner = window.prompt(
      `Assign owner to ${selected.length} lead${selected.length === 1 ? '' : 's'}:`,
      selected[0]?.lead.leadOwnerName ?? '',
    )
    if (!owner?.trim()) return
    const match = crmOwnerOptions.find((o) => o.label.toLowerCase() === owner.trim().toLowerCase())
    void (async () => {
      if (apiMode && match && /^[0-9a-f-]{36}$/i.test(match.value)) {
        const { bulkAssignLeadsApi } = await import('../../services/api/crmApi')
        const res = await bulkAssignLeadsApi(selected.map((r) => r.lead.id), match.value)
        showToast(`${res.data.processed} leads assigned`, res.data.failed ? 'warning' : 'success')
        return
      }
      let updated = 0
      for (const row of selected) {
        const result = match
          ? await resolveStoreAction(assignLead(row.lead.id, match.value))
          : await resolveStoreAction(updateLead(row.lead.id, { leadOwnerName: owner.trim() }))
        if (result.ok) updated += 1
      }
      showToast(`${updated} lead${updated === 1 ? '' : 's'} assigned to ${owner.trim()}`, 'success')
    })()
  }
  ...
  return (
    <>
      ...
      >
        <EnterpriseRegisterTableShell>
          <CrmLeadsTable
            ...
            onEdit={(row) => navigate(routes.edit(row.lead.id))}
            onDelete={openDeleteModal}
            onCreateOpportunity={(row) => {
              const { customerId, id: leadId } = row.lead
              if (!customerId) return
              navigate(`/crm/opportunities/new?customerId=${customerId}&leadId=${leadId}`)
            }}
            onScheduleActivity={(row) => setFollowUpLeadId(row.lead.id)}
            onCreateQuotation={(row) => {
              const { customerId } = row.lead
              if (!customerId) return
              navigate(`/crm/quotations/new?customerId=${customerId}`)
            }}
            ...
            onBulkChangeOwner={bulkAssignLeads}
      ...
      />
      <QuickFollowUpDrawer
        open={Boolean(followUpLeadId)}
        onClose={() => setFollowUpLeadId(null)}
        context={followUpCtx}
        onCreated={() => showToast('Activity scheduled', 'success')}
      />
      <LogActivityDrawer
