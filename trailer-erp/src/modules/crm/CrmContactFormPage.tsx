...
export function CrmContactFormPage() {
  ...
  const submit = handleSubmit(async (data) => {
    const validation = codeSeriesRef.current?.validateBeforeSave(data.contactCode, {
    ...
    const payload = {
      contactCode: data.contactCode.trim(),
    ...
    codeSeriesRef.current?.confirmSaved(data.contactCode)
    setAttachments([])
    reset({
      contactCode: '',
      customerId: prefillCustomerId,
  ...
  const commandBar = isEdit && existing ? (
    <ErpCardCommandBar
      inline
      homeActions={[
        { id: 'save', label: 'Save', icon: Save, primary: true, onClick: () => void submit() },
        { id: '360', label: 'View 360', icon: User, onClick: () => navigate(`/crm/contacts/${id}`) },
      ]}
      moreActions={[
        ...(customer ? [{ id: 'company', label: 'Company 360', icon: Building2, onClick: () => navigate(entity360CustomerPath(customer.id)) }] : []),
      ]}
    />
  ) : undefined
