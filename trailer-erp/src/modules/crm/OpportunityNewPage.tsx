...
export function OpportunityNewPage() {
  ...
  const [opportunityName, setOpportunityName] = useState(
    lead?.productRequirement?.trim() || lead?.prospectName?.trim() || '',
  )
  const [productRequirement, setProductRequirement] = useState(lead?.productRequirement ?? '')
      ...
      try {
        ...
        const r = await resolveStoreAction(
          createOpportunity({
            ...
            productRequirement: productRequirement.trim() || syncedLines.map((l) => l.productOrItem).filter(Boolean).join('; ') || opportunityName.trim(),
            lines: syncedLines,
            stage,
  ...
  return (
        ...
        <ErpStickySaveBar
          ...
          hint={(
            <span className="text-[12px] text-erp-muted">
              {completionPercent}% complete · Ctrl+S Create · Ctrl+Shift+S Save &amp; Close · Alt+N Save &amp; New
            </span>
          )}
        />
      )}
    >
      ...
      >
        ...
        <ErpFieldRow label="Opportunity Name" required>
          <Input
            value={opportunityName}
            onChange={(e) => setOpportunityName(e.target.value)}
            placeholder="e.g. 26 KL ISO Tank — Chemical Fleet"
            required
            className="erp-input"
          />
        </ErpFieldRow>
        ...
        <ErpFieldRow label="Expected Close Date" required>
      ...
      <ErpCardSection
        ...
        subtitle="Search products, then fine-tune qty and pricing."
        icon={ClipboardList}
        accent="teal"
        collapsible
        defaultOpen
      ...
      <ErpCardSection
        ...
        defaultOpen
      >
        <ErpFieldRow label="Expected Value (₹)" required readOnly>
          <Input type="text" value={formatCrmCurrency(dealValue)} readOnly className="erp-input" />
        </ErpFieldRow>
        ...
        <ErpFieldRow label="Weighted Value" readOnly>
          <Input type="text" value={formatCrmCurrency(weighted)} readOnly className="erp-input" />
        </ErpFieldRow>
