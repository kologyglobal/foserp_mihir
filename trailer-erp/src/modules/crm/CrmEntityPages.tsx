...
export function CrmCustomersPage() {
  ...
  const companyKpiStrip = useMemo<EnterpriseKpiItem[]>(
    () => [
      { id: 'total', label: 'Total Companies', value: kpis.totalCompanies, icon: KPI_ICON_PRESETS.open, accent: 'blue', context: 'Portfolio size', updatedAt: Date.now() },
      {
        ...
        accent: 'green',
        context: 'With open opportunities',
      ...
      },
      {
        ...
        accent: 'blue',
        context: `${kpis.openOpportunities} active opportunities`,
      ...
      },
      {
        ...
        accent: 'red',
        context: 'Needs sales action today',
      ...
      },
      {
        ...
        icon: KPI_ICON_PRESETS.pipeline,
        accent: 'green',
        context: 'Across portfolio',
        updatedAt: Date.now(),
      },
      {
        id: 'quotes',
        label: 'Quotation Value',
        value: formatCompactCurrency(kpis.quotationValue),
      ...
      },
      {
        ...
        icon: KPI_ICON_PRESETS.revenue,
        accent: 'amber',
        context: 'Click to filter',
        onClick: () => patchFilters({ outstandingAr: true }),
        updatedAt: Date.now(),
      },
    ],
    [kpis, patchFilters],
  ...
  function openCompanyPreview(row: (typeof sorted)[0]) {
    ...
    openDetailPanel({
      ...
      links: [{ label: 'Open Customer 360', href: `/entity360/customers/${customer.id}` }],
      ...
      actions: [
        { label: 'Customer 360', onClick: () => navigate(`/entity360/customers/${customer.id}`), primary: true },
  ...
  return (
    <>
      <OperationalPageShell
        title="Companies"
        description="Manage customers, pipeline, follow-ups and receivables by company"
        favoritePath="/crm/customers"
        badge="CRM"
        variant="dynamics"
        autoBreadcrumbs={false}
        breadcrumbs={crmModuleBreadcrumbs('Companies', '/crm/customers')}
        commandBar={
          <ErpCommandBar
            inline
            sticky={false}
      ...
      >
        <EnterpriseRegisterTableShell>
          <CrmCompaniesTable
            ...
            onOpen360={(id) => navigate(`/entity360/customers/${id}`)}
  ...
  function openContactPreview(row: EnrichedContactRow) {
    ...
    openDetailPanel({
      ...
      timeline: nextFu
        ...
        : [],
      links: [{ label: 'Open Customer 360', href: `/entity360/customers/${contact.customerId}` }],
      ...
      actions: [
        { label: 'Customer 360', onClick: () => navigate(`/entity360/customers/${contact.customerId}`), primary: true },
