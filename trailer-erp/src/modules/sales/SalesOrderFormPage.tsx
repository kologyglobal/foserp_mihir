...
export function SalesOrderEditPage() {
  ...
  if (!id || !so) {
    return (
      <div className="erp-page flex flex-col items-center justify-center gap-3 p-12 text-center">
        ...
        <AppLink to="/sales/orders" className="text-sm font-semibold text-erp-primary">Back to sales orders</AppLink>
      </div>
    )
  }
    ...
    return (
      <div className="erp-page flex flex-col items-center justify-center gap-3 p-12 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          <AppLink to="/sales/orders" className="text-sm font-semibold text-erp-primary">Back to sales orders</AppLink>
          <AppLink to={`/sales/orders/${so.id}`} className="text-sm font-semibold text-erp-primary">View order</AppLink>
        </div>
      </div>
    )
  }
  ...
  function handleSave(mode: 'save' | 'close' = 'save') {
    ...
    const r = updateDraft(draftSo.id, {
    ...
    if (r.ok) {
      navigate(mode === 'close' ? '/sales/orders' : `/sales/orders/${draftSo.id}`)
      return
    }
  ...
  const commandBar = (
    <ErpCardCommandBar
      inline
      homeActions={[
        { id: 'save', label: 'Save', icon: Save, primary: true, onClick: () => handleSave('save') },
        { id: '360', label: 'View 360', icon: Building2, onClick: () => navigate(`/sales/orders/${so.id}`) },
      ]}
      moreActions={[
        { id: 'list', label: 'All Sales Orders', icon: ClipboardList, onClick: () => navigate('/sales/orders') },
      ]}
    />
  )
  ...
  const factBox = (
    ...
    >
      <EnterpriseFormContextPanel
        ...
        actions={[
          { id: 'save', label: 'Save Changes', icon: Save, primary: true, onClick: () => handleSave('save'), disabled: isSubmitting },
          { id: '360', label: 'Order 360', icon: FileText, onClick: () => navigate(`/sales/orders/${so.id}`) },
        ]}
      />
      {so.quotationNo ? (
  ...
  return (
    <>
      <SalesCardFormShell
        ...
        favoritePath={`/sales/orders/${so.id}/edit`}
        breadcrumbs={salesBreadcrumbs(
          { label: 'Sales Orders', to: '/sales/orders' },
          { label: so.salesOrderNo, to: `/sales/orders/${so.id}` },
          { label: 'Edit' },
        )}
        ...
        onSaveShortcut={() => handleSave('save')}
        onSaveCloseShortcut={() => handleSave('close')}
        footer={(
          <ErpStickySaveBar
            isSubmitting={isSubmitting}
            submitLabel="Save Changes"
            cancelTo={`/sales/orders/${so.id}`}
