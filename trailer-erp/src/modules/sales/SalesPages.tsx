    ...
    <div className="erp-page">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        {quotation.status === 'approved' && quotation.isLatestRevision && !quotation.salesOrderId && (
          <Button
            ...
            onClick={() => {
              ...
              if (r.ok && r.salesOrderId) navigate(`/sales/orders/${r.salesOrderId}`)
        ...
        {quotation.salesOrderId && (
          <AppLink to={`/sales/orders/${quotation.salesOrderId}`}>View Sales Order {quotation.salesOrderNo}</AppLink>
        )}
      </div>
...
export function SalesOrderListPage({ crmMode = false }: { crmMode?: boolean } = {}) {
  ...
  function openSalesOrderPreview(so: SalesOrder) {
    ...
    openDetailPanel({
      ...
      actions: [
        ...
        ...(so.status === 'open' ? [{ label: 'Edit', onClick: () => navigate(`/sales/orders/${so.id}/edit`) }] : []),
      ],
    })
  }
  ...
  return (
    <OperationalPageShell
      title={crmMode ? 'CRM Sales Orders' : 'Sales Orders'}
      description={
        crmMode
          ? 'Orders from the CRM pipeline — complete draft handover here, then execute in Sales.'
          : 'Manage customer orders created from approved quotations or direct sales order entry.'
      }
      favoritePath={crmMode ? '/crm/sales-orders' : '/sales/orders'}
      badge={crmMode ? 'CRM' : undefined}
      variant="dynamics"
      autoBreadcrumbs={false}
      breadcrumbs={crmMode
        ? crmModuleBreadcrumbs('CRM Sales Orders', '/crm/sales-orders')
        : salesModuleBreadcrumbs('Sales Orders', '/sales/orders')}
      commandBar={
        crmMode ? (
          <ErpCommandBar
            inline
      ...
      filterBar={
        ...
        <SmartFilterBar
          chips={[
            ...(statusFilter ? [{ id: 'status', label: formatStatus(statusFilter) }] : []),
            ...(sourceFilter ? [{ id: 'source', label: `Source: ${sourceFilter}` }] : []),
            ...(search ? [{ id: 'search', label: `Search: ${search}` }] : []),
          ]}
          onRemoveChip={(id) => {
          ...
          onClearAll={() => { setStatusFilter(''); setSourceFilter(''); setSearch(''); setSortBy('orderDate') }}
          savedView={savedViews.activeView}
          onSavedViewChange={savedViews.selectView}
          savedViews={savedViews.viewNames}
          resultCount={sorted.length}
        >
          <SearchInput value={search} onChange={setSearch} placeholder="Search SO, customer, product, PO…" className="w-full sm:w-64" />
          <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-9 w-40 text-[13px]">
            <option value="">All Statuses</option>
            <option value="open">Draft SO</option>
            <option value="confirmed">Confirmed</option>
            <option value="in_production">In Production</option>
            <option value="ready_dispatch">Dispatch Ready</option>
      ...
      {crmMode ? (
        <EnterpriseRegisterTableShell>
          <SalesOrdersTable
            ...
            emptyAction={
              sorted.length === 0 ? (
                <div className="flex flex-wrap justify-center gap-2">
                  <button type="button" className="erp-btn erp-btn--primary text-[13px]" onClick={() => navigate('/sales/orders/new')}>
                    New Sales Order
                  </button>
            ...
            onEdit={(row) => navigate(`/sales/orders/${row.id}/edit`)}
