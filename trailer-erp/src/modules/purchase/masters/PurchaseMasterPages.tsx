...
export function PurchaseMasterFormPage() {
  ...
  return (
    <ErpFormShell
      ...
      breadcrumbs={masterBreadcrumbs(isEdit ? `Edit ${existing?.name ?? ''}` : 'New')}
      ...
      footerActions={(
        <ErpStickySaveBar
          ...
          actions={(
            <ErpButtonGroup>
              <ErpButton type="button" variant="ghost" onClick={() => navigate(`/purchase/masters/${slug}`)}>Cancel</ErpButton>
              <ErpButton type="submit" variant="primary" disabled={isSubmitting}>{isSubmitting ? 'Saving…' : 'Save'}</ErpButton>
            </ErpButtonGroup>
          )}
        />
      )}
    >
      <ErpFormStatusStrip items={[
        ...
        { label: 'Created by', value: existing?.createdBy ?? session.name, tone: 'neutral' },
      ]} />
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_240px]">
        <ErpCardSection
          title="Master Entry"
          subtitle="All purchase master fields on one screen."
          dense
        >
          <label className="block text-sm">
            <span className="font-medium text-erp-text">Code</span>
            <Input className="mt-1" value={code} disabled={existing?.systemControlled} onChange={(e) => setCode(e.target.value)} />
          </label>
          <label className="block text-sm">
            <span className="font-medium text-erp-text">Name</span>
            <Input className="mt-1" value={name} onChange={(e) => setName(e.target.value)} />
          </label>
          <label className="block text-sm">
            <span className="font-medium text-erp-text">Status</span>
            <Select wrapClassName="mt-1 w-full" value={status} onChange={(e) => setStatus(e.target.value as 'active' | 'inactive')}>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </Select>
          </label>
          {configFields.map((field) => (
            <label key={field.key} className={cn('block text-sm', field.type === 'textarea' && 'md:col-span-2')}>
              {field.type !== 'boolean' ? <span className="font-medium text-erp-text">{field.label}</span> : null}
              <div className={cn('mt-1', field.type === 'boolean' && 'mt-0')}>
                <MasterFieldInput field={field} value={attrs[field.key] ?? ''} onChange={(v) => setAttrs((prev) => ({ ...prev, [field.key]: v }))} />
              </div>
            </label>
          ))}
          <label className="block text-sm md:col-span-2">
            <span className="font-medium text-erp-text">Description</span>
            <Textarea className="mt-1" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} />
          </label>
          <label className="block text-sm md:col-span-2">
            <span className="font-medium text-erp-text">Notes</span>
            <Textarea className="mt-1" rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Internal notes" />
          </label>
        </ErpCardSection>
        <PurchaseMasterContextPanel catalog={catalog} entry={existing} pendingCreatedBy={session.name} />
      </div>
    </ErpFormShell>
  )
}
        ...
        >
          <DetailSection title="Basic Information">
            <DetailGrid>
              ...
              <DetailField label="Created on" value={new Date(entry.createdAt).toLocaleString('en-IN')} />
              <DetailField label="Created by" value={entry.createdBy ?? '—'} />
              <DetailField label="Last modified" value={`${new Date(entry.updatedAt).toLocaleString('en-IN')}${entry.modifiedBy ? ` · ${entry.modifiedBy}` : ''}`} />
          ...
          <DetailSection title="Configuration">
            <DetailGrid>
              {Object.entries(entry.attributes).map(([k, v]) => (
                <DetailField key={k} label={fieldLabel(k, catalog)} value={String(v ?? '—')} />
              ))}
            </DetailGrid>
          </DetailSection>
          {usage > 0 ? (
