...
export const useCrmStore = create<CrmState>()(
  persist(
    (set, get): CrmState => ({
      ...
      getTemplate: (id) => get().quotationTemplates.find((t) => t.id === id),
      ...
      createOpportunity: (input) => {
        ...
        if (input.leadId) {
          ...
          if (
            lead.opportunityId
            || lead.stage === 'converted_to_opportunity'
            || lead.lifecycleStatus === 'converted'
          ) {
            return { ok: false, error: 'Lead is already converted to an opportunity' }
          }
        }
        const audit = stampCreated()
      ...
      createQuotationFromOpportunity: (opportunityId, templateId, unitPrice, customLines) => {
        const opp = get().getOpportunity(opportunityId)
        if (!opp) return { ok: false, error: 'Opportunity not found' }
        ...
        const paymentTerms = paymentDefault.text
        const deliveryTerms = deliveryDefault.text
      ...
      convertQuotationDocumentToSalesOrder: (documentId, handover) => {
        ...
        logActivity(get, set, {
          ...
          subject: 'CRM handover complete',
          description: `Quotation ${salesQuo?.quotationNo ?? doc.quotationId} Rev ${doc.revisionNo} converted to Sales Order ${soNo}.`,
          quotationId: doc.quotationId,
          opportunityId: doc.opportunityId ?? undefined,
