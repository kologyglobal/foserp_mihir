...
export function normalizeLead(lead: Lead): Lead {
  ...
  // Empty string must not win over fallbacks (API create used to return leadOwnerName: "")
  const ownerName =
    lead.leadOwnerName?.trim()
    || lead.salesOwner?.trim()
    || (ownerId ? getLeadUser(ownerId)?.name : undefined)
    || 'Unassigned'
