...
/** Active opportunity stages from CRM Master Setup (falls back to defaults). */
export function resolveOpportunityStages(): OpportunityStageOption[] {
  const entries = getCrmMasterEntries('opportunity-stages', true)
...
export function openOpportunityStages(): OpportunityStageOption[] {
  return resolveOpportunityStages().filter((s) => !CLOSED_OPPORTUNITY_STAGES.has(s.id) && s.id !== 'on_hold')
}

/** Pipeline stages for Dynamics detail stepper */
export function buildOpportunityPipelineStages(stage: OpportunityStage) {
  const open = openOpportunityStages()
