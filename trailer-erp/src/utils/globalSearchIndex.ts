...
export function buildMasterSearchIndex(sources: MasterSearchSources): GlobalSearchIndexEntry[] {
  const out: GlobalSearchIndexEntry[] = []
  ...
  for (const customer of sources.customers) {
    ...
    pushEntry(
      out,
      {
        ...
        href: customer360Path(customer.id),
        group: 'master',
      },
      [
  ...
  for (const contact of sources.customerContacts) {
    ...
    pushEntry(
      out,
      {
        ...
        href: customer360Path(contact.customerId),
        group: 'master',
      },
...
export function buildGlobalSearchIndex(sources: GlobalSearchSources): GlobalSearchIndexEntry[] {
  return [
    ...buildPageSearchIndex(),
    ...buildMasterSearchIndex(sources.masters),
...
export function runGlobalSearch(query: string, sources: GlobalSearchSources, limit = 20): GlobalSearchHit[] {
  return searchGlobalIndex(buildGlobalSearchIndex(sources), query, limit)
}
