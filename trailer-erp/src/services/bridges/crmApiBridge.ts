...
function upsertLead(lead: Lead): void {
  useSalesStore.setState((s) => ({ leads: [lead, ...s.leads.filter((l) => l.id !== lead.id)] }))
}
...
export async function syncAllCrmFromApi(): Promise<void> {
  ...
  useCrmStore.setState({
    ...
    activities,
    followUps,
    quotationDocuments: quotationData.quotationDocuments,
    quotationTemplates,
  })
...
export async function apiCreateLead(input: Record<string, unknown>): Promise<StoreActionResult & { leadId?: string }> {
  return withSubmitLock(lockKey('lead:create'), async () => {
    try {
      ...
      const ownerName = String(input.leadOwnerName ?? '').trim()
      const ownerId = resolveOwnerId(input.leadOwnerId as string | undefined) ?? res.data.leadOwnerId
      // Keep list-column "Lead Owner" populated even if create DTO omitted resolved names
      upsertLead({
        ...res.data,
        leadOwnerId: res.data.leadOwnerId || ownerId || '',
        leadOwnerName: res.data.leadOwnerName?.trim() || ownerName || getStoredSession()?.user.name || '',
        salesOwner: res.data.salesOwner?.trim() || ownerName || getStoredSession()?.user.name || '',
      })
...
export async function apiUpdateOpportunity(
  id: string,
  patch: Partial<Pick<Opportunity, 'opportunityName' | 'value' | 'probability' | 'expectedCloseDate' | 'priority' | 'ownerId' | 'productRequirement' | 'contactId' | 'customerId' | 'lines' | 'stage' | 'status' | 'locationId'>>,
): Promise<StoreActionResult> {
  return withSubmitLock(lockKey('opportunity:update', id), async () => {
    try {
      ...
      if (patch.stage) {
        const pipeline = await getDefaultPipeline()
        const stage = pipeline.stages.find((s) => s.slug === patch.stage)
        if (stage) payload.stageId = stage.id
      }
      if (patch.ownerId) payload.ownerId = resolveOwnerId(patch.ownerId)
