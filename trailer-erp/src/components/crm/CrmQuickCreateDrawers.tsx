...
import type { CrmActivityType, OpportunityPriority, OpportunityStage } from '../../types/crm'
...
export function NewContactDrawer({
  ...
  return (
    <CrmDrawerShell
      ...
      onClose={onClose}
      footer={
        <Button type="submit" form="crm-new-contact-form" className="w-full">
          Create Contact
        </Button>
      }
    >
...
export function NewOpportunityDrawer({
  ...
  return (
    <CrmDrawerShell
      ...
      subtitle="Create a pipeline opportunity"
      onClose={onClose}
      width="lg"
      footer={
    ...
    >
      <form id="crm-new-opp-form" onSubmit={submit} className="crm-drawer-form">
        ...
        <FormField label="Opportunity name" required>
          <Input value={opportunityName} onChange={(e) => setOpportunityName(e.target.value)} required />
        </FormField>
...
export function LogActivityDrawer({
  ...
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const isLeadContext = Boolean(context?.leadId)
  ...
  function submit(e: React.FormEvent) {
    ...
    void (async () => {
      try {
        ...
        if (!r.ok) {
          setError(r.error ?? formatApiError('Failed to log activity'))
          return
        }
        onClose()
        setSubject('')
  ...
  return (
    <CrmDrawerShell
      ...
      placement="modal"
      title="Log Activity"
      subtitle={context?.leadName ?? 'Record a call, meeting, or note'}
      onClose={onClose}
      footer={
        <Button type="submit" form="crm-log-activity-form" className="w-full">
          Log Activity
        </Button>
      }
    >
