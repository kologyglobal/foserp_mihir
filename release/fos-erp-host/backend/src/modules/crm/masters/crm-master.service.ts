...
export async function listAllMastersForSync(tenantId: string) {
  ...
  const { CRM_MASTER_SEED_ROWS } = await import('../../../../prisma/crmMasterSeedData.js')
  const ensureKinds = ['payment-terms', 'industries', 'opportunity-priorities', 'lost-reasons', 'commercial-terms', 'document-types'] as const
  ...
  // Keep CRM owners in sync with active tenant users (UUID codes for assign APIs)
