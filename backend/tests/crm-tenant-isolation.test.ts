...
describe.skipIf(!runLive)('CRM tenant isolation', () => {
  ...
  it('dashboard metrics are tenant-scoped', async () => {
    ...
    expect(res.body.data.panels).toBeDefined()
    expect(typeof res.body.data.panels.pendingApprovalCount).toBe('number')
    expect(Array.isArray(res.body.data.panels.pendingApprovalQuotations)).toBe(true)
  })

  it('sales forecast is tenant-scoped', async () => {
    const res = await authGet(tokenA, `${BASE_A}/forecast`)
    expect(res.status).toBe(200)
    expect(res.body.success).toBe(true)
    expect(typeof res.body.data.openCount).toBe('number')
    expect(typeof res.body.data.weightedForecast).toBe('number')
