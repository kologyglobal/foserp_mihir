...
async function bootstrap(): Promise<ReversalFixture> {
  const { hashPassword } = await import('../../src/utils/password.js')
  const pw = await hashPassword('Test@123')
  const slug = `ar-rcpt-rev-${Date.now()}`
  const tenant = await prisma.tenant.create({
    data: { name: 'AR Receipt Reversal Test', slug, email: `${slug}@test.com`, status: 'ACTIVE' },
  })
  const user = await prisma.user.create({
    data: {
      tenantId: tenant.id,
  ...
  const leRes = await request(app)
    ...
    .send({
      ...
      legalName: 'AR Receipt Reversal Co Pvt Ltd',
      displayName: 'AR Rcpt Rev Co',
      stateCode: '27',
    })
  ...
  const customer = await prisma.crmCompany.create({
    data: {
      ...
      name: 'Reversal Customer Pvt Ltd',
  ...
  const noReceiptReverse = await createUserWithPerms(
    ctx.tenantId,
    ctx.slug,
    FINANCE_PERMS.filter((p) => p !== 'finance.ar.receipt.reverse') as PermissionName[],
    'no-receipt-reverse',
  )
  ...
  return {
    ...
    noReceiptReverseToken: noReceiptReverse.token,
    legalEntityId,
...
async function cleanupTenant(tenantId: string) {
  ...
  // Clear document → voucher FKs, then voucher ↔ voucher reversal FKs, before deletes.
...
async function postReceipt(fx: ReversalFixture, bankCashAmount: string): Promise<{
  receiptId: string
  ...
  const posted = await request(app)
    ...
    .set('Authorization', `Bearer ${fx.token}`)
  expect(posted.status).toBe(200)
  const receipt = await prisma.customerReceipt.findUniqueOrThrow({ where: { id: receiptId } })
  return {
    receiptId,
    creditOpenItemId: posted.body.data.creditOpenItemId as string,
    voucherId: receipt.accountingVoucherId as string,
...
function reverseAllocation(fx: ReversalFixture, receiptId: string, batchId: string, reason: string, key: string, token = fx.token) {
  return request(app)
...
function reverseReceipt(fx: ReversalFixture, receiptId: string, reason: string, key: string, token = fx.token) {
  return request(app)
    .post(`/api/v1/t/${fx.slug}/accounting/receivables/receipts/${receiptId}/reverse`)
    .set('Authorization', `Bearer ${token}`)
    .set('Idempotency-Key', key)
    .send({ reason })
}
...
describe.skipIf(!dbAvailable)('Finance Phase 3B6/3D — AR receipt reversal', () => {
  ...
  it('reverses an allocation: balances restored, lines REVERSED, no new voucher', async () => {
    ...
    const rev = await reverseAllocation(fx, rcpt.receiptId, batchId, 'wrong invoice', `rev-${Date.now()}`)
    ...
    expect(receipt.allocatedAmount.toFixed(4)).toBe('0.0000')
    expect(receipt.unallocatedAmount.toFixed(4)).toBe(inv.outstanding)

    const lines = await prisma.customerReceiptAllocation.findMany({ where: { batchId } })
    ...
    const audit = await prisma.auditLog.count({
      where: { tenantId: fx.tenantId, action: 'CUSTOMER_RECEIPT_ALLOCATION_REVERSED', entityId: batchId },
    })
    expect(audit).toBe(1)
  })
  ...
  it('allocation reverse is idempotent for same key and rejects mismatch', async () => {
    ...
    const first = await reverseAllocation(fx, rcpt.receiptId, batchId, 'duplicate entry', key)
    expect(first.status).toBe(200)
    expect(first.body.data.idempotentReplay).toBe(false)

    const second = await reverseAllocation(fx, rcpt.receiptId, batchId, 'duplicate entry', key)
    expect(second.status).toBe(200)
    expect(second.body.data.idempotentReplay).toBe(true)

    const mismatch = await reverseAllocation(fx, rcpt.receiptId, batchId, 'a different reason', key)
  ...
  it('allocation reverse requires finance.ar.allocation.reverse permission', async () => {
    ...
    const batchId = alloc.body.data.batch.id as string
    const res = await reverseAllocation(fx, rcpt.receiptId, batchId, 'no perm', `rev-perm-${Date.now()}`, fx.noAllocReverseToken)
  ...
  it('document reverse is blocked while POSTED allocations remain', async () => {
    ...
    const res = await reverseReceipt(fx, rcpt.receiptId, 'blocked by allocation', `rev-blk-${Date.now()}`)
    expect(res.status).toBe(422)
    expect(res.body.code).toBe('CUSTOMER_RECEIPT_ALLOCATIONS_MUST_BE_REVERSED')
  })
  ...
  it('full flow: allocate → reverse alloc → reverse document; GL nets, number kept', async () => {
    ...
    const batchId = alloc.body.data.batch.id as string
    await reverseAllocation(fx, rcpt.receiptId, batchId, 'undo alloc', `rev5a-${Date.now()}`).expect(200)

    const res = await reverseReceipt(fx, rcpt.receiptId, 'bank credited in error', `rev5b-${Date.now()}`)
    expect(res.status).toBe(200)
    expect(res.body.data.idempotentReplay).toBe(false)
    expect(res.body.data.receipt.status).toBe('REVERSED')
    ...
    expect(reversalVoucher.reversalOfVoucherId).toBe(rcpt.voucherId)

    const originalVoucher = await prisma.accountingVoucher.findUniqueOrThrow({ where: { id: rcpt.voucherId } })
    ...
    expect(receipt.reversalVoucherId).toBe(reversalVoucherId)
    expect(receipt.receiptNumber).toBe(rcpt.receiptNumber)

    const net = await glNetByAccount(fx.tenantId, [rcpt.voucherId, reversalVoucherId])
    for (const [, value] of net) {
      expect(Math.abs(value)).toBeLessThan(0.0001)
    }

    const audit = await prisma.auditLog.count({
      where: { tenantId: fx.tenantId, action: 'CUSTOMER_RECEIPT_REVERSED', entityId: rcpt.receiptId },
    })
    expect(audit).toBe(1)
  })

  it('document reverse is idempotent on replay', async () => {
    const rcpt = await postReceipt(fx, '2500.0000')
    const first = await reverseReceipt(fx, rcpt.receiptId, 'duplicate receipt', `rev6a-${Date.now()}`)
    expect(first.status).toBe(200)
    expect(first.body.data.idempotentReplay).toBe(false)
    const firstVoucherId = first.body.data.reversalVoucherId as string

    const second = await reverseReceipt(fx, rcpt.receiptId, 'duplicate receipt', `rev6b-${Date.now()}`)
    expect(second.status).toBe(200)
    expect(second.body.data.idempotentReplay).toBe(true)
    expect(second.body.data.reversalVoucherId).toBe(firstVoucherId)

    const reversalVouchers = await prisma.accountingVoucher.count({
      where: { tenantId: fx.tenantId, reversalOfVoucherId: rcpt.voucherId },
    })
    expect(reversalVouchers).toBe(1)
  })

  it('document reverse requires finance.ar.receipt.reverse permission', async () => {
    const rcpt = await postReceipt(fx, '1500.0000')
    const res = await reverseReceipt(fx, rcpt.receiptId, 'no perm', `rev7-${Date.now()}`, fx.noReceiptReverseToken)
    expect(res.status).toBe(403)
  })
})
