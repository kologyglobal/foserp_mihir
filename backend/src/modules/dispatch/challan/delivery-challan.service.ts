...
export async function updateChallan(req: Request, tenantId: string, id: string, input: UpdateChallanInput) {
  ...
  const row = await prisma.deliveryChallan.update({
    ...
    data: {
      ...
      eWayBillReference: input.eWayBillReference === undefined ? undefined : input.eWayBillReference,
      eWayBillDate: input.eWayBillDate !== undefined ? parseDate(input.eWayBillDate) : undefined,
