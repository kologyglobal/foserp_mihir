...
export async function updateSalesOrder(
  tenantId: string,
  id: string,
  data: Prisma.CrmSalesOrderUncheckedUpdateInput,
) {
  const existing = await findSalesOrderById(tenantId, id)
...
export async function softDeleteSalesOrder(tenantId: string, id: string, userId: string) {
  const existing = await findSalesOrderById(tenantId, id)
  if (!existing) return null
  return prisma.crmSalesOrder.update({
    where: { id },
