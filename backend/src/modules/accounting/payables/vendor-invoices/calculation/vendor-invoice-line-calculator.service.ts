...
import type {
  VendorInvoiceCalculationLineInput,
  VendorInvoiceLineDiscountType,
...
export interface RawVendorInvoiceLineAmounts {
  lineNumber: number
...
function applyLineDiscount(
  ...
  discountType: VendorInvoiceLineDiscountType | undefined,
  ...
  if (isNegative(value)) {
    errors.push(
      calcError(VENDOR_INVOICE_CALC_CODES.LINE_DISCOUNT_INVALID, 'Line discount cannot be negative', `lines[${lineNumber}].lineDiscountValue`),
    )
    return new Prisma.Decimal(0)
  }
  if (discountType === 'PERCENTAGE') {
    if (compare(value, 100) > 0) {
      errors.push(
        calcError(
          VENDOR_INVOICE_CALC_CODES.LINE_DISCOUNT_INVALID,
...
function validateRate(
  ...
  field: string,
  errors: VendorInvoiceValidationIssue[],
  code: string = VENDOR_INVOICE_CALC_CODES.TAX_RATE_INVALID,
): void {
...
export function computeRawLineAmounts(
...
): RawVendorInvoiceLineAmounts[] {
  const result: RawVendorInvoiceLineAmounts[] = []
  ...
  for (const line of lines) {
    ...
    if (!isPositive(qty)) {
      errors.push(calcError(VENDOR_INVOICE_CALC_CODES.QUANTITY_INVALID, 'Quantity must be greater than zero', `lines[${line.lineNumber}].quantity`))
    }
    if (isNegative(unitPrice)) {
      errors.push(calcError(VENDOR_INVOICE_CALC_CODES.UNIT_PRICE_INVALID, 'Unit price cannot be negative', `lines[${line.lineNumber}].unitPrice`))
    }
    ...
    if (hasExplicitRates) {
      ...
      if (!isZero(cgstRateOverride) && !isZero(igstRateOverride)) {
        warnings.push(
          calcWarning(
            VENDOR_INVOICE_CALC_CODES.EXPLICIT_RATE_SUPPLY_MISMATCH,
    ...
    validateRate(cessRate, `lines[${line.lineNumber}].cessRate`, errors, VENDOR_INVOICE_CALC_CODES.CESS_RATE_INVALID)

    const hsnCheck = validateHsnSac(line.hsnSacCode)
    if (!hsnCheck.valid) {
      errors.push(calcError(VENDOR_INVOICE_CALC_CODES.HSN_SAC_INVALID, hsnCheck.message ?? 'Invalid HSN/SAC', `lines[${line.lineNumber}].hsnSacCode`))
    } else if (hsnCheck.severity === 'warning') {
      warnings.push(
        calcWarning(
          VENDOR_INVOICE_CALC_CODES.HSN_SAC_LENGTH_WARNING,
