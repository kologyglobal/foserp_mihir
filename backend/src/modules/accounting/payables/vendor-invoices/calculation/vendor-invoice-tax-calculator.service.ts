...
export function determinePurchaseSupplyType(
  ...
  manualSupplyType: VendorInvoicePurchaseSupplyType | undefined,
  ...
  let derived: VendorInvoicePurchaseSupplyType
  if (company && reference) {
    ...
    if (!skipRequiredValidation) {
      errors.push(
        calcError(
          VENDOR_INVOICE_CALC_CODES.PLACE_OF_SUPPLY_REQUIRED,
  ...
  if (manualSupplyType && manualSupplyType !== derived) {
    errors.push(
      calcError(
        VENDOR_INVOICE_CALC_CODES.SUPPLY_TYPE_MISMATCH,
...
function resolveLineRates(line: RawVendorInvoiceLineAmounts, supplyType: VendorInvoicePurchaseSupplyType): RateSet {
...
function deriveTaxableFromInclusiveLocal(
  ...
  if (isZero(cessRate)) {
    ...
    if (compare(diff, INCLUSIVE_TOLERANCE) > 0) {
      errors.push(
        calcError(
          VENDOR_INVOICE_CALC_CODES.INCLUSIVE_TAX_MISMATCH,
  ...
  if (compare(diff, INCLUSIVE_TOLERANCE) > 0) {
    warnings.push(
      calcWarning(
        VENDOR_INVOICE_CALC_CODES.CESS_ON_INCLUSIVE_EXCEEDS_PRICE,
  ...
  switch (itcEligibility) {
    ...
    case 'PARTIALLY_ELIGIBLE': {
      ...
      if (percent == null || isNegative(percent) || compare(percent, 100) > 0) {
        errors.push(
          calcError(
            VENDOR_INVOICE_CALC_CODES.ITC_ELIGIBLE_PERCENT_INVALID,
    ...
    default:
      warnings.push(
        calcWarning(
          VENDOR_INVOICE_CALC_CODES.ITC_PENDING_REVIEW,
...
export interface VendorInvoiceLineTaxResult {
  lineNumber: number
...
export function computeLineTax(
  ...
  errors: VendorInvoiceValidationIssue[],
  warnings: VendorInvoiceValidationIssue[],
): VendorInvoiceLineTaxResult {
  const preTaxBase = roundTax(subtract(line.netBeforeHeaderDiscount, allocatedHeaderDiscount))
  const effectiveTaxTreatment = line.taxTreatment ?? headerTaxTreatment
...
function validateChargeRate(rate: Prisma.Decimal, field: string, errors: VendorInvoiceValidationIssue[]): void {
  if (compare(rate, 0) < 0 || compare(rate, 100) > 0) {
    errors.push(calcError(VENDOR_INVOICE_CALC_CODES.TAX_RATE_INVALID, 'Tax rate must be between 0 and 100', field))
  }
}
