...
export function calculateTds(
  ...
  const { mode, taxableAmount, invoiceGrandTotal } = input

  if (mode === 'NOT_APPLICABLE') {
    return { tdsBaseAmount: ZERO, tdsAmount: ZERO, estimatedTdsAmount: ZERO, vendorPayableAmount: invoiceGrandTotal }
  }

  const rate = toDecimal(input.rate ?? '0')
  if (isNegative(rate) || compare(rate, 100) > 0) {
    errors.push(calcError(VENDOR_INVOICE_CALC_CODES.TDS_RATE_INVALID, 'TDS rate must be between 0 and 100', 'tdsRate'))
  }
  ...
  if (input.baseOverride != null && input.baseOverride !== '') {
    ...
    if (isNegative(base)) {
      errors.push(calcError(VENDOR_INVOICE_CALC_CODES.TDS_BASE_INVALID, 'TDS base override cannot be negative', 'tdsBaseOverride'))
      base = taxableAmount
    }
  }
  ...
  if (mode === 'AT_INVOICE') {
    const vendorPayableAmount = roundTax(subtract(invoiceGrandTotal, computedTds))
    if (isNegative(vendorPayableAmount)) {
      errors.push(
        calcError(VENDOR_INVOICE_CALC_CODES.TDS_EXCEEDS_GRAND_TOTAL, 'TDS amount cannot exceed the invoice grand total', 'tdsAmount'),
      )
      return { tdsBaseAmount: base, tdsAmount: ZERO, estimatedTdsAmount: ZERO, vendorPayableAmount: invoiceGrandTotal }
    }
  ...
  warnings.push(
    calcWarning(
      VENDOR_INVOICE_CALC_CODES.TDS_AT_PAYMENT_NOTICE,
      'TDS is withheld at payment time — invoice posting will not reduce the vendor payable amount',
      'tdsRecognitionMode',
    ),
  )
  return { tdsBaseAmount: base, tdsAmount: ZERO, estimatedTdsAmount: computedTds, vendorPayableAmount: invoiceGrandTotal }
}
