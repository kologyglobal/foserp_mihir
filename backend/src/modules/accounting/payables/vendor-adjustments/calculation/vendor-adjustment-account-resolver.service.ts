 ...
 * in priority order: explicit `configuration.accounts` / line `offsetAccountId` override →
 * tenant `DefaultAccountMapping` → unresolved.
 *
 * `buildRequiredAccountComponents` is pure/sync (override-only) — reused directly by the
 * sync calculation helper for tests/pure previews. `resolveVendorAdjustmentAccounts` wraps it
...
import type {
  ...
  VendorAdjustmentAccountSource,
  VendorAdjustmentCalculationConfiguration,
...
const DEFAULT_MAPPING_BY_COMPONENT: Partial<Record<VendorAdjustmentAccountComponent, DefaultAccountMappingKey>> = {
  LINE_OFFSET: 'PURCHASE',
 ...
 */
export function computeRecoverableInputTaxByComponent(totals: VendorAdjustmentCalculationTotals): RecoverableInputTaxByComponent {
...
export interface BuildRequiredAccountComponentsParams {
  ...
  configuration: VendorAdjustmentCalculationConfiguration | undefined
 ...
 * `configuration.accounts` (header) and per-line `offsetAccountId` (already resolved onto
 * `amountsResult.lines[].offsetAccountId`). Anything not covered by an override comes back
 * UNRESOLVED; `resolveVendorAdjustmentAccounts` upgrades those via DefaultAccountMapping lookups.
 */
export function buildRequiredAccountComponents(params: BuildRequiredAccountComponentsParams): VendorAdjustmentResolvedAccount[] {
  const { amountsResult, configuration, tdsMode } = params
  const overrides = configuration?.accounts ?? {}
  const totals = amountsResult.totals
  const results: VendorAdjustmentResolvedAccount[] = []
  ...
  for (const line of amountsResult.lines) {
    ...
    if (line.offsetAccountId) {
      results.push(componentRef('LINE_OFFSET', line.lineNumber, true, 'LINE_OVERRIDE', line.offsetAccountId, null, null))
    } else if (headerDebitOverride) {
      results.push(
        componentRef('LINE_OFFSET', line.lineNumber, true, 'EXPLICIT', headerDebitOverride.id, headerDebitOverride.code, headerDebitOverride.name),
      )
    } else {
      results.push(componentRef('LINE_OFFSET', line.lineNumber, true, 'UNRESOLVED', null, null, null))
    }
  }

  const recoverable = computeRecoverableInputTaxByComponent(totals)
  const inputTaxComponents: Array<{ component: VendorAdjustmentAccountComponent; amount: string; override: typeof overrides.inputCgst }> = [
  ...
  if (amountsResult.isRcm) {
    ...
    const rcmComponents: Array<{ component: VendorAdjustmentAccountComponent; amount: string; override: typeof overrides.rcmCgstPayable }> = [
...
function issueForUnresolved(entry: VendorAdjustmentResolvedAccount): { code: string; message: string } {
  if (entry.component === 'LINE_OFFSET') {
    return {
      code: VENDOR_ADJUSTMENT_CALC_CODES.ACCOUNT_NOT_CONFIGURED,
...
async function validateResolvedAccounts(
  ...
  entries: VendorAdjustmentResolvedAccount[],
): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (!account) {
      entry.isValid = false
      entry.issueCode = VENDOR_ADJUSTMENT_CALC_CODES.ACCOUNT_NOT_FOUND
    ...
    if (account.isGroup) {
      entry.isValid = false
      entry.issueCode = VENDOR_ADJUSTMENT_CALC_CODES.ACCOUNT_IS_GROUP
      entry.issueMessage = `Account ${account.accountCode} is a group account and cannot be posted to`
      continue
    }
    if (!account.isActive) {
      entry.isValid = false
      entry.issueCode = VENDOR_ADJUSTMENT_CALC_CODES.ACCOUNT_INACTIVE
...
export interface ResolveVendorAdjustmentAccountsParams {
  tenantId: string
  legalEntityId: string
  amountsResult: VendorAdjustmentAmountsCalculationResult
  input: { configuration?: VendorAdjustmentCalculationConfiguration; tdsTreatment?: import('@prisma/client').VendorAdjustmentTdsTreatment }
  tdsMode?: TdsRecognitionMode
  includeDbLookups: boolean
}

export async function resolveVendorAdjustmentAccounts(params: ResolveVendorAdjustmentAccountsParams): Promise<VendorAdjustmentAccountReadiness> {
  const tdsMode =
    params.tdsMode ??
    (params.input.tdsTreatment === 'ADD_TDS_LIABILITY' || params.input.tdsTreatment === 'REVERSE_TDS_LIABILITY'
      ? 'AT_INVOICE'
      : 'NOT_APPLICABLE')
 ...
 */
export function finalizeAccountReadiness(resolved: VendorAdjustmentResolvedAccount[]): VendorAdjustmentAccountReadiness {
  ...
  const issues = resolved
    ...
    .map((e) => calcError(e.issueCode ?? VENDOR_ADJUSTMENT_CALC_CODES.ACCOUNT_NOT_CONFIGURED, e.issueMessage ?? `Account not resolved for ${e.component}`, e.component))
