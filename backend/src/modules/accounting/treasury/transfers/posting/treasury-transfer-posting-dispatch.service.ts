...
import { buildTreasuryTransferDispatchEventKey, buildTreasuryTransferPostingRequest } from './treasury-transfer-posting-builder.service.js'
...
export function assertDispatchPermission(req: Request): void {
  if (!hasPerm(req, 'finance.treasury.transfer.dispatch')) {
    throw new AuthorizationError('Missing permission: finance.treasury.transfer.dispatch')
  }
}

export async function dispatchTreasuryTransfer(
  req: Request,
  tenantId: string,
  transferId: string,
  body: { expectedUpdatedAt: string },
) {
  assertDispatchPermission(req)
  ...
  if ((existing.status === 'IN_TRANSIT' || existing.status === 'COMPLETED') && existing.sourceVoucherId && existing.sourcePostingEventId) {
  ...
  const validated = await validateTreasuryTransferForReadyToPostAction(tenantId, transferId, body.expectedUpdatedAt, 'IN_TRANSIT')
  ...
  const postingRequest = buildTreasuryTransferPostingRequest({
    ...
    eventKey: buildTreasuryTransferDispatchEventKey(transferId),
    eventType: 'TREASURY_TRANSFER_DISPATCHED',
    postingDate,
  })
  ...
  try {
    const posting = await post(postingRequest, postingContext, {
      ...
      afterAccounting: async ({ tx, context: txContext, eventId, voucherId }) => {
        ...
        const updated = await repo.finalizeDispatch(tx, {
          ...
          dispatchedById: txContext.userId ?? null,
        })
    ...
    await createAuditLog({
      ...
      entityId: transferId,
      action: 'TREASURY_TRANSFER_DISPATCHED',
