...
export function mapPurchaseOrderToDto(order: PurchaseOrder & { lines: PurchaseOrderLine[] }) {
  return {
    ...
    lines: order.lines.map((line) => ({
      ...
      quantity: num(line.quantity),
      uomQuantity: num((line as { uomQuantity?: unknown }).uomQuantity ?? line.quantity),
      uomConversionFactor: num((line as { uomConversionFactor?: unknown }).uomConversionFactor ?? 1),
      uomId: line.uomId,
      rate: num(line.rate),
      amount: num(line.amount),
