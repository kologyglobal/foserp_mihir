...
import { PurchaseRequisitionNotFoundError } from './purchase-requisition.errors.js'
...
export async function approvePurchaseRequisition(
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    await repo.resolvePendingApprovals(tenantId, id, 'APPROVED', actorId, input.remarks ?? null, tx)
