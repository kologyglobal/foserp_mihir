/**
 * cPanel / Hostinger entrypoint.
 * - Applies Prisma migrations
 * - Seeds once when the database has no tenants (SEED_ON_EMPTY=true)
 * - Builds dist/ when missing or incomplete (e.g. partial upload)
 * - Starts the API + SPA
 *
...
function distLooksComplete() {
  const required = [
    ...
    path.join(__dirname, 'dist', 'config', 'logger.js'),
    path.join(__dirname, 'dist', 'middleware', 'error.middleware.js'),
  ]
...
async function main() {
  ...
  console.log('[cpanel-start] dist/ still missing — starting with tsx src/server.ts')
