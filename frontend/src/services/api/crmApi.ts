...
export interface CrmDashboardMetricsDto {
  ...
  panels?: {
    ...
    pendingApprovalQuotations: Array<{
      id: string
      quotationId: string
...
export interface CrmSalesForecastDto {
  openCount: number
  pipelineValue: number
  weightedForecast: number
  ...
  byMonth: Array<{ month: string; label: string; pipeline: number; weighted: number; count: number }>
  byOwner: Array<{
    ownerId: string | null
    ownerName: string
    pipeline: number
...
export async function fetchCrmSalesForecast(params?: Record<string, string | undefined>) {
  return apiRequest<CrmSalesForecastDto>(`${tenantPath('/crm/forecast')}${buildQuery(params)}`)
}
