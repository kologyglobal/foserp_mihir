...
export function ContactImportDialog({ open, onClose, onImported }: ContactImportDialogProps) {
  const customers = useMasterStore((s) => s.customers)
  const contacts = useCrmStore((s) => s.contacts)
  const createContact = useCrmStore((s) => s.createContact)
  const fileRef = useRef<HTMLInputElement>(null)
  const [rows, setRows] = useState<ContactImportPreviewRow[]>([])
  ...
  function handleFile(file: File) {
    ...
    reader.onload = () => {
      ...
      const parsed = parseContactImportCsv(text, customers)
  ...
  function handleImport() {
      ...
      void (async () => {
        try {
          const payloadRows = validRows.map((r) => ({
            'Company Code': customers.find((c) => c.id === r.input.customerId)?.customerCode ?? '',
            'Company Name': r.companyLabel,
            'Contact Name': r.input.name,
            Designation: r.input.designation,
            Department: r.input.department,
            Email: r.input.email,
            Phone: r.input.phone,
            Primary: r.input.isPrimary ? 'yes' : 'no',
            Active: r.input.isActive ? 'yes' : 'no',
          }))
          const res = await importContactsApi(payloadRows, 'skip')
    ...
    const result = importContactRows(
      (input) => createContact(input) as { ok: boolean; error?: string; contactId?: string },
      contacts,
      validRows,
    )
    setImporting(false)
    ...
    if (result.failed === 0) {
      setResultMessage(`${result.imported} contact${result.imported === 1 ? '' : 's'} imported successfully.`)
      setRows([])
    } else {
      setResultMessage(
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[12px] text-erp-muted">
                  Includes {CONTACT_IMPORT_HEADERS.length} columns with two sample rows. Companies must already exist in CRM.
                </p>
                <ErpButton
                  type="button"
                  variant="primary"
          ...
          <section className="rounded-lg border border-erp-border bg-erp-surface-alt/40 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[12px] text-erp-muted">
                  Keep the header row. Required: Contact Name and Company Code or Company Name.
                </p>
