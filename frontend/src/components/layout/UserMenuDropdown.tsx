    ...
    <div ref={ref} className={cn('relative', className)}>
      <button
        ...
        className={cn(
          isSuite
            ? 'd365-suite-user'
            : 'flex h-8 items-center gap-2 rounded-[4px] border border-erp-border bg-erp-surface pl-1 pr-2 transition-colors hover:border-erp-border-strong hover:bg-erp-surface-alt',
        )}
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      >
        <span
          className={cn(
            isSuite
              ? 'd365-suite-avatar'
              : 'flex h-6 w-6 items-center justify-center rounded-[4px] bg-erp-primary text-white',
          )}
        >
        ...
        <span className={cn('hidden text-left lg:block', !isSuite && 'sm:block')}>
          <span
            className={cn(
              isSuite ? 'd365-suite-user-name block' : 'erp-type-caption-strong block leading-tight',
            )}
          >
            {name}
          </span>
      ...
      {open && (
        ...
        >
          <div
            className={cn(
              isSuite ? 'd365-user-menu-header' : 'border-b border-erp-border px-3 py-2.5',
            )}
          >
            <p
              className={cn(
                isSuite ? 'd365-user-menu-name' : 'text-[13px] font-semibold text-erp-text',
              )}
            >
              {name}
            </p>
            {email ? (
              <p className={cn(isSuite ? 'd365-user-menu-email' : 'text-[11px] text-erp-muted')}>
                {email}
              </p>
            ) : null}
            <p
              className={cn(
                isSuite ? 'd365-user-menu-role' : 'mt-0.5 text-[11px] text-erp-muted',
              )}
            >
              {roleLabel}
            </p>
          ...
          <button
            ...
            className={cn(
              isSuite
                ? 'd365-user-menu-item d365-user-menu-item-danger'
                : 'flex w-full items-center gap-2 px-3 py-2 text-left text-[13px] text-red-600 hover:bg-red-50',
            )}
