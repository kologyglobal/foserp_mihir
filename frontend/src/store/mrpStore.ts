...
export const useMrpStore = create<MrpState>()(
  ...
  addSalesOrderFromQuotation: (input) => {
    if (isApiMode()) {
      return {
        ok: false,
        error: 'Use quotation convert or apiCreateSalesOrder in API mode — local SO create is demo-only',
      }
    }
  ...
  updateSalesOrderDraft: (salesOrderId, patch) => {
    if (isApiMode()) {
      return { ok: false, error: 'Use apiUpdateSalesOrder in API mode — local draft update is demo-only' }
    }
  ...
  deleteSalesOrderDraft: (salesOrderId) => {
    if (isApiMode()) {
      return { ok: false, error: 'Use apiDeleteSalesOrder in API mode — local draft delete is demo-only' }
    }
    const so = get().getSalesOrder(salesOrderId)
    if (!so) return { ok: false, error: 'Sales order not found' }
    if (so.status !== 'open') {
  ...
  confirmSalesOrder: (salesOrderId) => {
    if (isApiMode()) {
      return { ok: false, error: 'Use apiConfirmSalesOrder in API mode — local confirm is demo-only' }
    }
    const so = get().getSalesOrder(salesOrderId)
    if (!so) return { ok: false, error: 'Sales order not found' }
