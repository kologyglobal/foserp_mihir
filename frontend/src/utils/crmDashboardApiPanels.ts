...
import type { CrmDashboardMetrics, CrmQuotationApprovalRow } from './crmMetrics'
...
export function applyApiDashboardPanelOverlay(
  ...
  const pendingApprovalQuotations: CrmQuotationApprovalRow[] = (api.panels.pendingApprovalQuotations ?? []).map(
    (row) => ({
      id: row.id,
      ...
      status: row.status as CrmQuotationApprovalRow['status'],
  ...
  return {
    ...
    quotationsPending: api.panels.pendingApprovalCount ?? pendingApprovalQuotations.length,
  }
}
