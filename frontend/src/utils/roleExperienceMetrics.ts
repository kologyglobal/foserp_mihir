...
export function computeAllRoleKpis(): Record<string, RoleKpi> {
  ...
  return {
    ...
    wipValue: kpi('wipValue', 'WIP Value', formatMetricCurrency(exec.wipValue), 'blue', '/work-orders'),
    ...
    invoiceValue: kpi('invoiceValue', 'Invoiced YTD', formatMetricCurrency(exec.invoiceValue), 'blue', '/invoices/register'),
    outstanding: kpi('outstanding', 'Outstanding AR', formatMetricCurrency(exec.outstanding), exec.outstanding > 0 ? 'amber' : 'green', '/invoices/register'),
    paymentReceived: kpi('paymentReceived', 'Collected', formatMetricCurrency(exec.paymentReceived), 'green'),
    delayedOrders: kpi('delayedOrders', 'Delayed Orders', exec.delayedOrders, exec.delayedOrders ? 'red' : 'green', '/reports/sales/open-orders'),
    ...
    costVariance: kpi('costVariance', 'Cost Variance', formatMetricCurrency(exec.costVariance), 'amber', '/costing'),
    activeWo: kpi('activeWo', 'Active WO', prod.running + prod.late + prod.qcHolds, 'blue', '/work-orders'),
    lateWo: kpi('lateWo', 'Late WO', prod.late, prod.late ? 'red' : 'green', '/reports/production/wo-status'),
    ...
    delayedPo: kpi('delayedPo', 'Delayed PO', delayedPo, delayedPo ? 'red' : 'green', '/reports/purchase/delayed-po'),
    grnPending: kpi('grnPending', 'Draft GRN', grns.filter((g) => g.status === 'draft').length, 'blue', '/purchase/grns'),
    ...
    woMaterialShort: kpi('woMaterialShort', 'WO Material Gaps', mrp.woShortages.length, mrp.woShortages.length ? 'red' : 'green', '/work-orders'),
    submittedBom: kpi('submittedBom', 'BOM Awaiting Approval', submittedBom, submittedBom ? 'amber' : 'green', '/masters/bom'),
    ...
    pendingDrawings: kpi('pendingDrawings', 'Draft BOM', bomHeaders.filter((b) => b.status === 'draft').length, 'blue', '/masters/bom'),
    ...
    negativeStock: kpi('negativeStock', 'Negative Stock Items', negativeStock, negativeStock ? 'red' : 'green', '/reports/inventory/negative-stock'),
    ...
    slowMoving: kpi('slowMoving', 'Low Stock SKUs', stock.filter((s) => s.isLowStock).length, 'amber', '/reports/inventory/slow-moving'),
    qcIncoming: kpi('qcIncoming', 'Incoming QC Queue', incomingQc, incomingQc ? 'amber' : 'green', '/quality/incoming'),
    runningWo: kpi('runningWo', 'WO In Production', prod.running, 'green', '/work-orders'),
    todayJobCards: kpi('todayJobCards', 'Active Job Cards', prod.todayJobCards.length, 'blue', '/production/job-cards'),
    ...
    paymentPending: kpi('paymentPending', 'Unpaid Invoices', invMetrics.unpaidCount, 'amber', '/invoices/register'),
    overdueInvoices: kpi('overdueInvoices', 'Overdue Invoices', overdueInvoices, overdueInvoices ? 'red' : 'green', '/invoices/register'),
    dispatchUnbilled: kpi('dispatchUnbilled', 'Dispatched — Not Invoiced', dispatchUnbilled, dispatchUnbilled ? 'amber' : 'green', '/invoices/register'),
  }
}
