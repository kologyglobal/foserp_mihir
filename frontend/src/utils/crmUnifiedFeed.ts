...
import { isValidTimestamp } from './dates/format'
...
export interface UnifiedFeedItem {
  ...
  title: string
  body?: string
  /** ISO-ish string for sorting (date or datetime); empty when unknown */
  at: string
  meta?: string
  badge?: string
...
export function buildUnifiedFeed(input: {
  ...
  input.notes.forEach((note, index) => {
    const at = isValidTimestamp(note.createdAt) ? String(note.createdAt) : ''
    items.push({
