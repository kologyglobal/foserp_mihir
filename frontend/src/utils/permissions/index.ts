...
function mapBackendRoleToErpRole(roles: string[]): ErpRole {
  ...
  if (normalized.some((r) => r === 'admin' || r.includes('super admin') || r.includes('tenant admin'))) {
    return 'admin'
  }
...
function isCrmShellPath(pathname: string): boolean {
  return (
    pathname === '/crm'
...
export {
  ...
  hasBudgetingPermission,
  useBudgetingPermissions,
...
export {
  ...
  canPurchaseRoute,
  canViewPurchaseNavItem,
  getPurchasePermissionDenialReason,
  isPurchasePath,
  purchaseActionGate,
  resolvePurchaseRoutePermission,
  usePurchasePermissions,
  withPurchaseActionGate,
  PURCHASE_PERMISSIONS,
  type PurchasePermission,
} from './purchase'

export {
  canAccessInventoryShell,
  canInventoryPermission,
  canViewInventoryNavItem,
  inventoryActionGate,
  isInventoryPath,
  resolveInventoryRoutePermission,
...
export {
  ...
  canManufacturingPermission,
  canManufacturingRoute,
  isManufacturingPath,
  useManufacturingPermissions,
