...
import type { VendorPaymentAccountingPreview } from '@/types/moneyOut'
import { parseDecimal } from '../moneyOutUi'

/** Read-only accounting preview built by the server (validation.accountingPreview). No UI recalculation. */
export function VendorPaymentAccountingPreviewTable({
  preview,
}: {
  preview: VendorPaymentAccountingPreview | null | undefined
}) {
  if (!preview) {
    ...
    <div className="overflow-x-auto">
      <div className="mb-2 flex flex-wrap items-center gap-3 text-[12px]">
        ...
        <span className="text-erp-muted">
          Vendor payable debit: {formatCurrency(parseDecimal(preview.vendorPayableDebitAmount))}
        </span>
        <span className="text-erp-muted">
          Payment account credit: {formatCurrency(parseDecimal(preview.paymentAccountCreditAmount))}
        </span>
      </div>
      <table className="w-full min-w-[640px] text-left text-[12px]">
        ...
        <tfoot>
          <tr className="border-t border-erp-border font-semibold">
            ...
            <td className="py-2 pr-2 text-right tabular-nums">{formatCurrency(parseDecimal(preview.debitTotal))}</td>
            <td className="py-2 text-right tabular-nums">{formatCurrency(parseDecimal(preview.creditTotal))}</td>
          </tr>
        </tfoot>
      </table>
      <p className="mt-2 text-[11px] text-erp-muted">
        Preview is server-built. Posting uses the same formula. Allocation of this payment against invoices creates no
        GL — it only updates subledger open-item balances.
      </p>
    </div>
  )
}
