...
import { getPayableAgeingReport } from '@/services/bridges/payablesApiBridge'
import { resolveLegalEntityId } from '@/services/bridges/financeApiBridge'
import type { PayableAgeingBasis, PayableAgeingReportDto } from '@/types/moneyOut'
...
export function AgeingPage() {
  ...
  const [report, setReport] = useState<PayableAgeingReportDto | null>(null)
  const [loading, setLoading] = useState(true)
  const [basis, setBasis] = useState<PayableAgeingBasis>('due_date')
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      setReport(await getPayableAgeingReport({ legalEntityId: resolveLegalEntityId(), ageingBasis: basis }))
    } catch (e) {
  ...
  if (!perms.canView) {
    return (
      <MoneyOutWorkspaceShell title="Ageing">
    ...
    return (
      <MoneyOutWorkspaceShell title="Ageing">
        <p className="text-[13px] text-erp-muted">
          AP ageing requires API mode (<code>VITE_USE_API=true</code>).
        </p>
      </MoneyOutWorkspaceShell>
    )
  }
      ...
      commandBar={
        <div className="flex flex-wrap gap-2">
          <Select
            ...
            value={basis}
            onChange={(e) => setBasis(e.target.value as PayableAgeingBasis)}
          >
            <option value="due_date">Due date basis</option>
            <option value="document_age">Document age basis</option>
          </Select>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <p className="text-[12px] text-erp-muted">
            ...
            <strong>{formatCurrency(parseDecimal(report.totals.baseOutstandingAmount))}</strong> ({report.totals.openItemCount}{' '}
            items)
          </p>
          <div className="h-64">
