...
import type { ChequeValidationSnapshot } from '../api/treasury-cheque.types'

export function ChequeValidationPanel({ snapshot }: { snapshot: ChequeValidationSnapshot | null | undefined }) {
  if (!snapshot) {
    return (
      <div className="rounded-lg border border-dashed border-erp-border bg-erp-surface-alt/40 p-4 text-[12px] text-erp-muted">
        Not yet validated. Run Validate to check account, amount, and counterpart rules.
      </div>
    )
  }
