...
import { DisputeDrawer, DisputeStatusBadge, ReceivablesWorkspaceTabs } from '@/components/accounting/receivables'
...
export function DisputesPage() {
  ...
  const [editing, setEditing] = useState<CustomerDispute | null>(null)
  ...
  if (!perms.canView) {
    return (
      <OperationalPageShell
        ...
        badge="Accounting"
        title="Disputes"
        breadcrumbs={[{ label: 'Accounting', to: '/accounting' }, { label: 'Disputes' }]}
  ...
  return (
    <OperationalPageShell
      ...
      badge="Accounting"
      title="Disputes"
      description="Invoice disputes and resolution tracking."
      breadcrumbs={[
    ...
    >
      ...
      <div className="mb-3 mt-3 space-y-3">
        <SearchInput
          ...
          placeholder="Search dispute, customer, invoice…"
        />
      ...
      <EnterpriseRegisterTableShell>
        ...
        {!loading && rows.length === 0 ? (
          <div className="p-6">
            <EmptyState icon={FileWarning} title="No disputes" description="Raise a dispute from this register or from an invoice row." />
          </div>
        ) : null}
        {!loading && rows.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="erp-table w-full min-w-[1120px] text-[13px]">
              <thead>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-erp-border/70 hover:bg-erp-surface-alt/40">
                    ...
                    <td className="px-3 py-2">
                      {r.salesOrders?.length
                        ? r.salesOrders.map((order, index) => (
                            <span key={order.id}>
                              {index > 0 ? ', ' : null}
                              <TableLink to={`/crm/sales-orders/${order.id}`}>{order.number}</TableLink>
                            </span>
                          ))
                        : '—'}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <DisputeStatusBadge status={r.status} />
                    </td>
                  </tr>
                ))}
              </tbody>
