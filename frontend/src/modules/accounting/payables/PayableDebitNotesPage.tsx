...
import type { PayableDebitNote, PayableFilter } from '@/types/payables'
...
export function PayableDebitNotesPage() {
  const navigate = useNavigate()
  const perms = usePayablesPermissions()
  const [filter, setFilter] = useState<PayableFilter>({ ...DEFAULT_PAYABLE_FILTER, debitNoteTab: 'all' })
  const [rows, setRows] = useState<PayableDebitNote[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [applyTarget, setApplyTarget] = useState<PayableDebitNote | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      setRows(await getDebitNotes({ search: filter.search, debitNoteTab: filter.debitNoteTab }))
    } catch (e) {
      setError(e instanceof PayablesServiceError ? e.message : 'Failed to load debit notes')
      setRows([])
    } finally {
      setLoading(false)
    }
  }, [filter.search, filter.debitNoteTab])
  ...
  const kpiStrip = useMemo(
    () => [
      { id: 'total', label: 'Debit notes', value: rows.length, accent: 'blue' as const },
      {
        id: 'unapplied',
      ...
      { id: 'posted', label: 'Posted', value: rows.filter((r) => r.status === 'Posted').length, accent: 'green' as const },
    ],
    [rows],
  )

  const openApply = (dn: PayableDebitNote) => {
    if (!perms.canApplyDebitNote) {
      notify.error('You do not have permission to apply debit notes.')
      return
    }
    setApplyTarget(dn)
    setInvoiceId(dn.referenceInvoiceId ?? '')
    setApplyAmount(String(dn.remainingAmount))
  }
  ...
  const confirmApply = async () => {
    ...
    setApplying(true)
    try {
      await applyDebitNoteDemo(applyTarget.id, invoiceId, amount)
      notify.success(`Debit note ${applyTarget.debitNoteNumber} applied (demo).`)
      setApplyTarget(null)
      await load()
    } catch (e) {
  ...
  if (!perms.canView) {
    return (
      <OperationalPageShell
        ...
        badge="Accounting"
        title="Debit Notes"
        breadcrumbs={[{ label: 'Accounting', to: '/accounting' }, { label: 'Debit Notes' }]}
  ...
  return (
    <OperationalPageShell
      ...
      badge="Accounting"
      title="Debit Notes"
      description="Vendor debit notes and application to open invoices — demo UI only."
      breadcrumbs={[
      ...
      autoBreadcrumbs={false}
      favoritePath="/accounting/payables/debit-notes"
      kpiStrip={kpiStrip}
      commandBar={
        <ErpCommandBar
          inline
    ...
    >
      <PayablesWorkspaceTabs active="debit_notes" />
      <div className="mb-3 mt-3 space-y-3">
        <SearchInput
          value={filter.search}
          onChange={(v) => setFilter((f) => ({ ...f, search: v }))}
          placeholder="Search debit note, vendor, reference invoice…"
        />
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={cn(
                ...
                filter.debitNoteTab === tab.id ? 'bg-sky-50 text-sky-900 ring-sky-300' : 'bg-white text-erp-muted ring-erp-border',
              )}
              onClick={() => setFilter((f) => ({ ...f, debitNoteTab: tab.id }))}
            >
              {tab.label}
            </button>
          ))}
      ...
      <EnterpriseRegisterTableShell>
        ...
        {!loading && error ? (
          <div className="p-6">
            <EmptyState icon={FileText} title="Could not load debit notes" description={error} />
          </div>
        ) : null}
        {!loading && !error && rows.length === 0 ? (
          <div className="p-6">
            <EmptyState icon={FileText} title="No debit notes" />
          </div>
        ) : null}
        {!loading && !error && rows.length > 0 ? (
            ...
            <table className="erp-table w-full min-w-[1100px] text-[13px]">
              <thead>
                <tr className="border-b bg-erp-surface-alt/60 text-[11px] uppercase text-erp-muted">
                  <th className="px-3 py-2 text-left">Debit note</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-erp-border/70 hover:bg-erp-surface-alt/40">
                    <td className="px-3 py-2 font-medium">{r.debitNoteNumber}</td>
                    <td className="px-3 py-2 tabular-nums">{r.debitNoteDate}</td>
                    ...
                    <td className="px-3 py-2">
                      {r.referenceInvoiceId ? (
                        <TableLink to={`/accounting/payables/invoices/${r.referenceInvoiceId}`}>
                    ...
                    <td className="px-3 py-2">
                      <PayableDebitNoteStatusBadge status={r.status} />
                    </td>
                    <td className="px-3 py-2 text-right">
                      {r.remainingAmount > 0 && !['Cancelled', 'Draft'].includes(r.status) && perms.canApplyDebitNote ? (
                        <button
                          type="button"
      ...
      <PayableConfirmModal
        ...
        title="Apply debit note to invoice"
        description={applyTarget ? `${applyTarget.debitNoteNumber} · Remaining ${formatCurrency(applyTarget.remainingAmount)}` : ''}
      ...
      >
        <div className="mt-4 space-y-3">
          ...
          <p className="text-[11px] text-erp-muted">Demo mode — reduces invoice outstanding via debit note offset.</p>
        </div>
      </PayableConfirmModal>
    </OperationalPageShell>
  )
