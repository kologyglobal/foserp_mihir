...
import {
  exportManufacturingCostData,
  getManufacturingCostPrintPreview,
  getManufacturingCostReports,
  ManufacturingAccountingServiceError,
} from '@/services/accounting/manufacturingAccountingService'
import type { ManufacturingCostReportCard } from '@/types/manufacturingAccounting'
...
const CATEGORIES = ['All', 'Costing', 'WIP', 'Variance', 'Analysis'] as const

export function ManufacturingCostingReportsPage() {
  const perms = useManufacturingAccountingPermissions()
  const [catalog, setCatalog] = useState<ManufacturingCostReportCard[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      setCatalog(await getManufacturingCostReports())
      setLoadState('ready')
    } catch {
      setLoadState('error')
    }
  ...
  useEffect(() => {
    ...
    void getManufacturingCostPrintPreview(selected.name).then((p) => setPreviewRows(p.rows))
  }, [selected])
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    ...
    try {
      const result = await exportManufacturingCostData({ reportName: selected.name, format, filter: {} })
      notify.success(`${result.fileName} generated (${result.rowCount} rows, demo).`)
    } catch (e) {
      notify.error(e instanceof ManufacturingAccountingServiceError ? e.message : 'Export failed')
    }
  }
  ...
  const handlePrint = async () => {
    ...
    try {
      const preview = await getManufacturingCostPrintPreview(selected.name)
      notify.info(`${preview.reportName} print preview ready — ${preview.rows.length} row(s) (demo).`)
    } catch (e) {
      notify.error(e instanceof ManufacturingAccountingServiceError ? e.message : 'Print failed')
    }
  }

  if (!perms.canViewReports) {
    return (
      <OperationalPageShell variant="dynamics" layout="enterprise" badge="Accounting" title="Manufacturing Reports" breadcrumbs={[...MFG_ACCOUNTING_BREADCRUMB, { label: 'Reports' }]} autoBreadcrumbs={false}>
  ...
  return (
    <OperationalPageShell
      ...
      layout="enterprise"
      badge="Accounting"
      title="Manufacturing Costing Reports"
      description="Report catalog with export and print preview (demo)."
      breadcrumbs={[...MFG_ACCOUNTING_BREADCRUMB, { label: 'Reports' }]}
      autoBreadcrumbs={false}
      favoritePath="/accounting/manufacturing/reports"
      showDescription
      commandBar={(
        <ErpCommandBar
          inline
    ...
    >
      <ManufacturingAccountingWorkspaceTabs active="reports" />
      <div className="space-y-3 p-4">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          <div className="space-y-2 lg:col-span-1">
            ...
            {visible.length === 0 && loadState !== 'loading' ? <ManufacturingAccountingEmptyState title="No reports match filters" /> : null}
          </div>
