    ...
    >
      <div className="mb-3 flex flex-wrap gap-2">
        {[
          ...
          { label: 'Approved (not posted)', value: summary.approved, tone: 'border-emerald-200 bg-emerald-50' },
        ].map((chip) => (
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-erp-border/70 hover:bg-slate-50">
                  ...
                  <td className="px-2 py-2">
                    <span className={`rounded px-2 py-0.5 text-[11px] font-medium ${statusTone(row.status)}`}>{row.status.replace(/_/g, ' ')}</span>
                  </td>
