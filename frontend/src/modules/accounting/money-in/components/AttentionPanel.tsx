...
export function AttentionPanel({
  ...
  const items = [
    readyToPost > 0 && {
      ...
      label: `${readyToPost} invoice${readyToPost === 1 ? '' : 's'} ready to post`,
      to: '/accounting/money-in/invoices?status=READY_TO_POST',
    },
    overdueCount > 0 && {
      icon: Clock,
      tone: 'text-rose-700 bg-rose-50 border-rose-200',
      label: `${overdueCount} overdue open item${overdueCount === 1 ? '' : 's'}`,
      to: '/accounting/money-in/outstanding',
    },
    exceptionCount > 0 && {
      icon: AlertTriangle,
      tone: 'text-orange-700 bg-orange-50 border-orange-200',
      label: `${exceptionCount} reconciliation exception${exceptionCount === 1 ? '' : 's'}`,
      to: '/accounting/money-in/reconciliation',
    },
  ...
  if (items.length === 0) {
    return (
      <div className="rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-[12px] text-emerald-800">
        No attention items — AR subledger is current.
      </div>
    )
  }
