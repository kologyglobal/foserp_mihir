...
import {
  exportBankCashData,
  getBankCashPrintPreview,
  getBankCashReports,
  BankCashServiceError,
} from '@/services/accounting/bankCashService'
import type { BankCashReportCard } from '@/types/bankCash'
import { useBankCashPermissions } from '@/utils/permissions/bankCash'
import { notify } from '@/store/toastStore'
import { BANK_CASH_BREADCRUMB, type LoadState } from './bankCashUi'
import { cn } from '@/utils/cn'

const CATEGORIES = ['All', 'Bank', 'Cash', 'Reconciliation', 'Compliance'] as const

export function BankCashReportsPage() {
  const perms = useBankCashPermissions()
  const [catalog, setCatalog] = useState<BankCashReportCard[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      setCatalog(await getBankCashReports())
      setLoadState('ready')
    } catch {
      setLoadState('error')
    }
  ...
  useEffect(() => {
    ...
    void getBankCashPrintPreview(selected.name).then((p) => setPreviewRows(p.rows))
  }, [selected])
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    ...
    try {
      const result = await exportBankCashData({ reportName: selected.name, format, filter: {}, includeAudit: false })
  ...
  const handlePrint = async () => {
    ...
    try {
      const preview = await getBankCashPrintPreview(selected.name)
      notify.info(`${preview.reportName} print preview ready — ${preview.rows.length} row(s) (demo).`)
    } catch (e) {
      notify.error(e instanceof BankCashServiceError ? e.message : 'Print failed')
    }
  }

  if (!perms.canViewReports) {
    return (
      <OperationalPageShell variant="dynamics" layout="enterprise" badge="Accounting" title="Bank & Cash Reports" breadcrumbs={[...BANK_CASH_BREADCRUMB, { label: 'Reports' }]} autoBreadcrumbs={false}>
  ...
  return (
    <OperationalPageShell
      ...
      breadcrumbs={[...BANK_CASH_BREADCRUMB, { label: 'Reports' }]}
      autoBreadcrumbs={false}
      favoritePath="/accounting/bank-cash/reports"
      showDescription
      commandBar={(
        <ErpCommandBar
          inline
    ...
    >
      <BankCashWorkspaceTabs active="reports" />
      <div className="space-y-3 p-4">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          <div className="space-y-2 lg:col-span-1">
            ...
            {visible.length === 0 && loadState !== 'loading' ? <BankCashEmptyState title="No reports match filters" /> : null}
          </div>
