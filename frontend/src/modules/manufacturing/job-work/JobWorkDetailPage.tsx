...
import { JobWorkStatusBadge } from '../ui'
...
const PRIMARY_TABS: Array<{ id: Tab; label: string }> = [
  ...
  { id: 'material_sent', label: 'Materials' },
  { id: 'receipts', label: 'Receipts' },
]

const MORE_TABS: Array<{ id: Tab; label: string }> = [
  { id: 'reconciliation', label: 'Reconciliation' },
  { id: 'invoice', label: 'Vendor Invoice' },
...
function SummaryTile({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="rounded-md border border-erp-border bg-erp-surface px-3 py-2">
      <div className="text-[10px] font-medium uppercase tracking-wide text-erp-muted">{label}</div>
...
export function JobWorkDetailPage() {
  ...
  useEffect(() => {
    ...
    if (t && ALL_TABS.some((x) => x.id === t)) setTab(t)
    if (action || t) {
  ...
  const canReconcile =
    !readOnly &&
    perms.canReconcileJobWork &&
    ['received', 'reconciliation_pending', 'partially_received'].includes(jobWork.status)
  ...
  const moreTabActive = MORE_TABS.some((t) => t.id === tab)

  const primaryAction: ErpCommandAction | undefined = canSend
  ...
  const moreActions: ErpCommandAction[] = [
    ...(!readOnly && jobWork.status === 'draft' && perms.canEditJobWork
      ...
      : []),
    ...(canReceive && primaryAction?.id !== 'receive'
      ? [{ id: 'receive', label: 'Receive', onClick: () => setDialog('receive') }]
      : []),
    ...(canReconcile && primaryAction?.id !== 'reconcile'
      ? [
          {
            id: 'reconcile',
            label: 'Reconcile',
    ...
    ...(canInvoice
      ...
      : []),
    ...(canClose && primaryAction?.id !== 'close'
      ? [{ id: 'close', label: 'Close', onClick: () => setDialog('close') }]
      : []),
    ...(canCancel ? [{ id: 'cancel', label: 'Cancel', onClick: () => setDialog('cancel') }] : []),
  ]
    ...
    >
      <div className="space-y-3">
        <div className="rounded-lg border border-erp-border bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-[14px] font-medium text-erp-text">
                {jobWork.process} · {jobWork.vendorName}
              </p>
            ...
            <JobWorkStatusBadge status={jobWork.status} />
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
          <SummaryTile label="Ordered" value={`${jobWork.orderedQty} ${jobWork.uom}`} />
          <SummaryTile label="Sent" value={jobWork.sentQty} />
          <SummaryTile label="Received" value={jobWork.receivedQty} />
          <SummaryTile label="Balance" value={jobWork.pendingQty} />
          <SummaryTile label="Expected Cost" value={formatCurrency(jobWork.expectedCost)} />
        </div>
        ...
        <div
          className="relative flex flex-wrap items-center gap-1 rounded-lg border border-erp-border bg-white p-1"
          role="tablist"
          aria-label="Job work tabs"
        >
          {PRIMARY_TABS.map((t) => (
            <button
              key={t.id}
          ...
          <section className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="rounded-lg border border-erp-border bg-white p-4">
              ...
              <ol className="list-decimal space-y-2 pl-4 text-[13px] text-erp-text">
                ...
                <li
                  className={
                    ['material_sent', 'partially_received', 'received', 'reconciliation_pending', 'closed'].includes(
                      jobWork.status,
                    )
                      ? 'text-emerald-700'
                      : ''
                ...
                <li
                  className={
                    ['partially_received', 'received', 'reconciliation_pending', 'closed'].includes(jobWork.status)
                      ? 'text-emerald-700'
                      : ''
                  }
                >
                ...
                <li
                  className={
                    ['reconciliation_pending', 'closed'].includes(jobWork.status) || jobWork.differenceApproved
                      ? 'text-emerald-700'
                      : ''
                  }
                >
                ...
                <li className={jobWork.invoiceStatus !== 'none' ? 'text-emerald-700' : ''}>
                  Link Vendor Invoice (placeholder)
                </li>
        ...
        {tab === 'material_sent' ? (
          <section className="space-y-3 rounded-lg border border-erp-border bg-white p-4">
        ...
        {tab === 'reconciliation' ? (
          <section className="space-y-3 rounded-lg border border-erp-border bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {canReconcile ? (
                <Button size="sm" onClick={() => setDialog('reconcile')}>
                  Reconcile
                </Button>
              ) : null}
            </div>
            {!recon || recon.lines.length === 0 ? (
              <p className="py-6 text-center text-[13px] text-erp-muted">
                No reconciliation data yet. Send and receive material first.
              </p>
            ) : (
              <>
                {(() => {
                  const sum = recon.lines.reduce(
                    (acc, l) => ({
                      sent: acc.sent + l.sent + l.additionalSent,
                      consumed: acc.consumed + l.consumed,
                      returned: acc.returned + l.returned + l.scrapReturned,
                      loss: acc.loss + l.processLoss,
                      pending: acc.pending + l.actualBalance,
                    }),
                    { sent: 0, consumed: 0, returned: 0, loss: 0, pending: 0 },
                  )
                  return (
                    <div className="rounded-md border border-erp-border bg-erp-surface-alt px-3 py-2.5">
                      ...
                      <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px] tabular-nums text-erp-text">
                        <span className="font-semibold">Sent {sum.sent}</span>
                        <span className="text-erp-muted">=</span>
                        <span>Consumed {sum.consumed}</span>
                        <span className="text-erp-muted">+</span>
                        <span>Returned {sum.returned}</span>
                        <span className="text-erp-muted">+</span>
                        <span>Process loss {sum.loss}</span>
                        <span className="text-erp-muted">+</span>
                        <span>Pending with vendor {sum.pending}</span>
                      </p>
                      {recon.unexplainedDifference !== 0 ? (
                        <p className="mt-1 text-[12px] font-semibold text-rose-700">
                          Unexplained difference: {recon.unexplainedDifference} — must be approved or resolved before
                          close.
                        </p>
                      ) : (
                        <p className="mt-1 text-[12px] font-medium text-emerald-700">No unexplained difference.</p>
                      )}
                    </div>
                  )
                })()}
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={busy} onClick={confirmDialog}>
              Confirm
            </Button>
          </div>
        }
      >
        {dialog === 'dispatch' ? (
          <div className="space-y-3">
            <p className="text-[13px] text-erp-muted">Send required material quantities to {jobWork.vendorName}.</p>
            <ul className="text-[12px]">
              {materials.map((m) => (
                <li key={m.id} className="flex justify-between border-b border-erp-border py-1">
                  <span>{m.materialName}</span>
                  <span className="tabular-nums">
                    {Math.max(0, m.requiredQty - m.sentQty)} {m.uom}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="grid gap-3">
            ...
            <FormField label="Invoice No">
              <Input
                ...
                placeholder={`PI-${jobWork.jwNumber}`}
              />
            </FormField>
