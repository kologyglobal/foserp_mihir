...
export function MobileQcListPage() {
  ...
  return (
    <>
      <MobilePageTitle title="Quality Inspections" subtitle={`${pending.length} pending`} />
      {pending.map((insp) => (
        <button
          key={insp.id}
          type="button"
        ...
        >
          ...
          <div className="text-sm">{insp.category} · {insp.woNo ?? insp.grnNo ?? insp.itemCode ?? '—'}</div>
        ...
        <div key={p.parameterId} className="mob-card mb-2">
          ...
          {p.parameterType === 'boolean' && (
            <div className="grid grid-cols-2 gap-2 mt-2">
              <button type="button" className="mob-btn mob-btn-primary" onClick={() => setParams((ps) => ps.map((x) => (x.parameterId === p.parameterId ? fillParam(x, true) : x)))}>
                Pass
              </button>
              <button type="button" className="mob-btn mob-btn-danger" onClick={() => setParams((ps) => ps.map((x) => (x.parameterId === p.parameterId ? fillParam(x, false) : x)))}>
                Fail
              </button>
            </div>
          )}
          {p.parameterType === 'numeric' && (
            <input
              ...
              onChange={(e) => setParams((ps) => ps.map((x) => (x.parameterId === p.parameterId ? fillParam(x, Number(e.target.value)) : x)))}
            />
          )}
