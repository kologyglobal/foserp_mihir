...
export function MobileShopFloorDemoPage() {
  const navigate = useNavigate()
  const jobCards = useOpenJobCards()

  return (
    <>
      <MobilePageTitle title="Shop Floor" subtitle="Demo job cards — enable API for live kiosk" />
      {jobCards.slice(0, 40).map((jc) => (
        ...
        >
          ...
          <div className="text-sm text-[#605e5c]">
            WO {jc.woNo} · Op seq {jc.sequenceNo}
          </div>
        </button>
      ))}
    </>
...
export function MobileJobCardDemoPage() {
  ...
  function act(action: 'start' | 'pause' | 'resume' | 'complete') {
    ...
    if (action === 'start' || action === 'resume') {
      const r = start(jc!.id, {
        assignedTeam: jc!.assignedTeam ?? 'Mobile Ops',
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        ...
        <button
          type="button"
          className="mob-btn mob-btn-secondary col-span-2"
          onClick={() => navigate(`/m/job-card/${jc.id}/daily-entry`)}
        >
          Daily Entry
        </button>
      </div>
...
export function MobileJobCardDailyEntryDemoPage() {
  const { id } = useParams<{ id: string }>()
  const jc = useWorkOrderStore((s) => s.jobCards.find((j) => j.id === id))
  ...
  return (
    <>
      ...
      <div className="mob-field mb-3 mt-3">
        <label>Remarks</label>
        <textarea className="w-full min-h-[80px] border rounded-lg px-3 py-2" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
      </div>
      <MobilePhotoCapture label="Photo evidence" onCapture={() => {}} />
      {msg ? <div className="mob-card text-sm mb-3">{msg}</div> : null}
      <MobileStickyActionBar>
