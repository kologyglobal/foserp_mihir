...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[var(--ds-z-modal,500)] flex items-center justify-center p-4" role="dialog" aria-modal>
      <button
        type="button"
      ...
      <div className={cn('relative w-full rounded-lg border border-[var(--dyn-border)] bg-[var(--dyn-bg-card)] shadow-lg', width)}>
        <div className="flex items-start justify-between gap-3 border-b border-[var(--dyn-border)] px-4 py-3">
          <div>
            <h2 className="ds-type-workspace-title font-semibold text-[var(--dyn-text)]">{title}</h2>
