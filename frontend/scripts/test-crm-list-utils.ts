...
const { sortSalesOrders, isSalesOrderOverdue } = await import('../src/utils/salesDashboardMetrics')
const { salesOrderFromApi } = await import('../src/services/bridges/salesOrderApiBridge')
...
let overdueCrash = false
try {
  // Simulate pre-normalize / hostile payload (null requiredDate) hitting list overdue check
  overdueCrash = isSalesOrderOverdue({
    id: 'x',
    salesOrderNo: 'SO-NULL',
    customerId: 'c1',
    productId: '',
    qty: 1,
    status: 'open',
    requiredDate: null as unknown as string,
    remarks: '',
...
check(16, 'isSalesOrderOverdue tolerates null/empty requiredDate (post-cleanup API rows)', !overdueCrash)

const mapped = salesOrderFromApi({
  id: 'so-1',
  salesOrderNo: 'SO-001',
  customerId: 'c1',
...
check(
  17,
  'salesOrderFromApi coerces null dates/product and keeps cleared quotationId',
  mapped.requiredDate === '' &&
    mapped.productId === '' &&
    mapped.quotationId === null &&
    mapped.opportunityId === null &&
    mapped.quotationNo === 'QT-KEPT' &&
    mapped.remarks === '',
)
