...
if (p2) {
  ...
  if (opp.ok && opp.opportunityId) {
    const q = crm.createQuotationFromOpportunity(opp.opportunityId, 'qtpl-iso-dry-bulk-25cbm', 2000000)
  ...
  if (openOpp.ok && openOpp.opportunityId) {
    const q2 = crm.createQuotationFromOpportunity(openOpp.opportunityId, 'qtpl-iso-dry-bulk-25cbm', 1500000)
    if (q2.documentId) {
