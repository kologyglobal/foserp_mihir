...
if (oppResult.ok && oppResult.opportunityId) {
  const q = useCrmStore.getState().createQuotationFromOpportunity(oppResult.opportunityId, 'qtpl-iso-dry-bulk-25cbm', 2_100_000)
...
if (highOpp.ok && highOpp.opportunityId) {
  const hq = useCrmStore.getState().createQuotationFromOpportunity(highOpp.opportunityId, 'qtpl-iso-dry-bulk-25cbm', 6_000_000)
