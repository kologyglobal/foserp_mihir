...
if (!pendingCrmDoc) {
  ...
  if (openOpp) {
    const q = crm.createQuotationFromOpportunity(openOpp.id, 'qtpl-iso-dry-bulk-25cbm', 2200000)
...
if (multiOpp.ok && multiOpp.opportunityId) {
  const q = crm.createQuotationFromOpportunity(multiOpp.opportunityId, 'qtpl-iso-dry-bulk-25cbm', 2400000)
  if (q.documentId) {
